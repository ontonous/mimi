...
| **标准库** | 22 模块: io, fs, net, json, csv, crypto, regex, template, array, iter 等 |
...
| `array` | `array.mimi` | 固定大小数组: fill, slice, rotate, binary_search |
| `iter` | `iter.mimi` | 迭代器: range, zip, enumerate, take, drop, chain |
...
| `mimi emit-cpp-bindings <file>` | 导出 C++ RAII 绑定 |
| `mimi emit-rust-bindings <file>` | 导出 Rust FFI 绑定 |
| `mimi emit-go-bindings <file>` | 导出 Go CGO 绑定 |
| `mimi emit-node-bindings <file>` | 导出 Node.js N-API 绑定 + TypeScript .d.ts |
| `mimi emit-java-bindings <file>` | 导出 Java JNI 绑定 |
| `mimi emit-py-bindings <file>` | 导出 Python pybind11 绑定 |
| `mimi bindgen <file> -o <dir>` | 一键生成所有语言绑定 |
| `mimi stat [path]` | 目录统计（文件、目录、扩展名） |
...
| 版本 | 亮点 |
|---|---|---|
| **v0.28.30** | Actor 字段原地 mutate + codegen map_get string ABI |
| **v0.28.29** | `from_json::<List<T>>` 返回可变 List |
| **v0.28.28** | Actor 方法可调用用户函数 |
| **v0.28.27** | 真实代码 codegen sprint: reduce/trait self/newtype |
| **v0.28.26** | 质量止血 + JSON 贯通: from_json/to_json List<RecordType> |
| **v0.28.25** | 包导入 + MimiSpec 语法清理 + prelude 自动加载 |
| **v0.28.24** | 标准库完整性 + 类型系统修复 |
| **v0.28.21** | Comptime/Quote codegen + LSP 补全/跳转/悬停 |
| **v0.28.20** | 并发原语: Mutex, Atomic, Channel |
| **v0.28.19** | Actor Codegen: 真实 mailbox + worker 线程 |
| **v0.28.18** | FFI 完整: 复杂 repr(C) 返回, 跨线程 callback |
| **v0.28.17** | CLI 一致性: 统一类型检查器, SAFETY 注释 59 处 |
| **v0.28.16** | Codegen 根基补强: shared/weak 生命周期, 字符串泄漏修复 |
| **v0.28.15** | 安全审查: 所有 #[ignore] 差距关闭, unsafe 审计 270 处 |
| **v0.28.14** | 诊断与格式化: 错误恢复, 多位置诊断, lint 扩展 |
| **v0.28.13** | 标准库扩展: mymath, array, iter + codegen 优化骨架 |
| **v0.28.12** | 包管理加固: add/install/remove/tree |
| **v0.28.11** | LSP: 悬停, 补全, 跳转定义, 重命名 |
| **v0.28.10** | Codegen 差距关闭: sort_str, const, Set<T>, from_json |
| **v0.28.9** | FFI 导出 wrapper + 多语言绑定生成 + repr(C) |
| **v0.28.8** | Codegen 质量重构 + helper 单元测试 + lexer/parse 双后端测试，clippy 清零 |
| **v0.28.7** | 多行语法 + mimi-make + mimi-lint + push() 修复 |
