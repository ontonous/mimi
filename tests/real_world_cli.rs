...
#[test]
fn canonical_scalar_ffi_f32_direct_auxiliary_cli_success_without_legacy() {
    if !can_link() {
        eprintln!("SKIP: cc not available");
        return;
    }
    let dir = std::env::temp_dir().join(format!(
        "mimi_ffi_cli_f32_direct_auxiliary_{}_{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
    ...
    fs::create_dir_all(&dir).expect("create direct f32 auxiliary directory");
    let c_path = dir.join("ffi.c");
    let library = dir.join("ffi.so");
    fs::write(
        &c_path,
        "float mir_ffi_f32(float value) { return value; }\n",
    )
    .expect("write direct f32 auxiliary C fixture");
    let compile_c = Command::new("cc")
        .args(["-shared", "-fPIC", "-O2"])
        .arg(&c_path)
        .arg("-o")
        .arg(&library)
        .output()
        .expect("compile direct f32 auxiliary C fixture");
    assert!(
        compile_c.status.success(),
        "{}",
        String::from_utf8_lossy(&compile_c.stderr)
    ...
    fs::write(
        ...
        r#"
extern "C" {
    func mir_ffi_f32(value: f32) -> f32;
}
func test_boundary() -> bool {
    mir_ffi_f32(1.5 as f32);
    true
}
func main() -> i64 { 0 }
"#,
    )
    .expect("write direct f32 auxiliary source");
    ...
    let test = Command::new(mimi_bin())
        ...
        .env("MIMI_FFI_LIB", &library)
        .output()
        .expect("spawn direct f32 auxiliary test");
    assert!(
        test.status.success(),
        "direct f32 auxiliary test failed:\n{}\n{}",
    ...
    let disasm = Command::new(mimi_bin())
        ...
        .arg(&source)
        .output()
        .expect("spawn direct f32 auxiliary disasm");
    assert!(
        disasm.status.success(),
        "direct f32 auxiliary disasm failed:\n{}\n{}",
    ...
    assert!(disasm_stdout.contains("function:test_boundary"));
    ...
    let mir = Command::new(mimi_bin())
        ...
        .arg(&source)
        .output()
        .expect("spawn direct f32 auxiliary MIR");
    assert!(
        mir.status.success(),
        "direct f32 auxiliary MIR failed:\n{}\n{}",
        String::from_utf8_lossy(&mir.stdout),
        String::from_utf8_lossy(&mir.stderr)
    );
    assert!(String::from_utf8_lossy(&mir.stdout).contains("function:test_boundary"));
    ...
    let receipt = Command::new(mimi_bin())
        ...
        .arg(&source)
        .output()
        .expect("spawn direct f32 auxiliary receipt");
    assert!(
        receipt.status.success(),
        "direct f32 auxiliary receipt failed:\n{}\n{}",
    ...
    let ffi_digest = manifest.get("ffi_digest").expect("direct f32 FFI digest");
    assert_eq!(ffi_digest.len(), 64);
    assert!(ffi_digest
        .chars()
        .all(|character| character.is_ascii_hexdigit()));
    let root_owners = manifest.get("root_owners").expect("direct f32 root owners");
    assert!(root_owners.contains("function:test_boundary"));
    ...
    let build_ir_runs = [
        Command::new(mimi_bin())
            ...
            .env_remove("MIMI_FFI_LIB")
            .output()
            .expect("spawn direct f32 auxiliary default IR build"),
        Command::new(mimi_bin())
            .current_dir(project_root())
            ...
            .env_remove("MIMI_FFI_LIB")
            .output()
            .expect("spawn direct f32 auxiliary explicit MIR IR build"),
    ];
    assert_eq!(
        build_ir_runs[0].stdout, build_ir_runs[1].stdout,
        "direct f32 no-contract IR must match between default and explicit MIR routes"
    );
    assert_eq!(
        build_ir_runs[0].stderr, build_ir_runs[1].stderr,
        "direct f32 no-contract IR diagnostics must match between routes"
    );
    for build_ir in &build_ir_runs {
        assert!(
            build_ir.status.success(),
            "direct f32 no-contract IR build failed:\n{}\n{}",
        ...
        assert!(ir.contains("mir_ffi_f32"), "{ir}");
...
#[test]
fn canonical_scalar_ffi_f64_imported_module_test_executes_external_call() {
    if !can_link() {
        eprintln!("SKIP: cc not available");
        return;
    }
    let dir = std::env::temp_dir().join(format!(
        "mimi_ffi_cli_f64_imported_test_exec_{}_{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
    ...
    fs::create_dir_all(&dir).expect("create imported f64 test execution directory");
    let c_path = dir.join("ffi.c");
    let library = dir.join("ffi.so");
    fs::write(
        &c_path,
    ...
    .expect("write imported f64 test execution C fixture");
    let compile_c = Command::new("cc")
        .args(["-shared", "-fPIC", "-O2"])
        .arg(&c_path)
        .arg("-o")
        .arg(&library)
        .output()
        .expect("compile imported f64 test execution C fixture");
    assert!(
        compile_c.status.success(),
        "{}",
        String::from_utf8_lossy(&compile_c.stderr)
...
"#,
    ...
    .expect("write imported f64 test execution module");
    let source = dir.join("main.mimi");
    fs::write(
        &source,
        r#"
use ffi_types
func test_boundary() -> bool { call(-7.5); seen() == 1 }
func main() -> i64 {
    call(-7.5);
    0
}
"#,
    ...
    .expect("write imported f64 test execution entry");
    ...
    let output = Command::new(mimi_bin())
        ...
        .env("MIMI_FFI_LIB", &library)
        .output()
        .expect("spawn imported f64 test execution route");
    assert!(
        output.status.success(),
        "imported f64 test execution failed:\n{}\n{}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    let stdout = String::from_utf8_lossy(&output.stdout);
...
#[test]
fn canonical_scalar_ffi_f64_imported_module_zero_arg_return_matches_explicit_mir() {
    if !can_link() {
        eprintln!("SKIP: cc not available");
        return;
    }
    let dir = std::env::temp_dir().join(format!(
        "mimi_ffi_cli_f64_imported_zero_arg_{}_{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
    ...
    fs::create_dir_all(&dir).expect("create imported f64 zero-arg directory");
    let c_path = dir.join("ffi.c");
    let library = dir.join("ffi.so");
    fs::write(
        &c_path,
        "#include <stdint.h>\ndouble imported_f64_seed(void) { return 7.5; }\nint64_t imported_f64_check(double value) { return value == 7.5 ? 42 : -1; }\n",
    )
    .expect("write imported f64 zero-arg C fixture");
    let compile_c = Command::new("cc")
        .args(["-shared", "-fPIC", "-O2"])
        .arg(&c_path)
        .arg("-o")
        .arg(&library)
        .output()
        .expect("compile imported f64 zero-arg C fixture");
    assert!(
        compile_c.status.success(),
        "{}",
        String::from_utf8_lossy(&compile_c.stderr)
...
pub type Scalar = f64
pub extern "C" {
    func imported_f64_seed() -> Scalar;
    func imported_f64_check(value: Scalar) -> i64;
}
pub func seed() -> Scalar { imported_f64_seed() }
pub func check(value: Scalar) -> i64 { imported_f64_check(value) }
"#,
    )
    .expect("write imported f64 zero-arg module");
    let source = dir.join("main.mimi");
    fs::write(
        &source,
        r#"
...
"#,
    ...
    .expect("write imported f64 zero-arg entry");
    ...
    for explicit_mir in [false, true] {
        ...
        let output = command
            ...
            .env("MIMI_FFI_LIB", &library)
            .output()
            .unwrap_or_else(|error| panic!("imported f64 zero-arg {explicit_mir}: {error}"));
        assert!(
            output.status.success(),
            "imported f64 zero-arg {explicit_mir} failed:\n{}\n{}",
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
        assert_eq!(output.stdout, b"8\n42\n");
...
#[test]
#[cfg(unix)]
fn canonical_scalar_ffi_f64_imported_libm_matches_cli_consumers() {
    if !can_link() {
        eprintln!("SKIP: cc not available");
        return;
    }
    ...
    fs::create_dir_all(&dir).expect("create imported libm f64 directory");
    fs::write(
...
pub func rounded_abs(value: Scalar) -> i64 { llrint(fabs(value)) }
"#,
    )
    .expect("write imported libm f64 module");
    let source = dir.join("main.mimi");
    fs::write(
        &source,
        r#"
use libm_float
func main() -> i64 {
    println(rounded_abs(-7.5))
    0
}
"#,
    )
    .expect("write imported libm f64 entry");
    let binary = dir.join("imported-libm-f64");
    ...
    for explicit_mir in [false, true] {
        ...
        assert!(
            run.status.success(),
            "imported libm f64 run {explicit_mir} failed:\n{}\n{}",
            String::from_utf8_lossy(&run.stdout),
            String::from_utf8_lossy(&run.stderr)
        );
        assert_eq!(run.stdout, b"8\n");
        ...
        assert!(
            verify.status.success(),
            "imported libm f64 verify {explicit_mir} failed:\n{}\n{}",
        ...
        assert!(
            build_ir.status.success(),
            "imported libm f64 IR {explicit_mir} failed:\n{}\n{}",
            String::from_utf8_lossy(&build_ir.stdout),
            String::from_utf8_lossy(&build_ir.stderr)
        );
        let ir = String::from_utf8_lossy(&build_ir.stdout);
        assert!(ir.contains("fabs"));
        assert!(ir.contains("llrint"));
        ...
        let build = build
            ...
            .output()
            .unwrap_or_else(|error| {
                panic!("imported libm f64 native build {explicit_mir}: {error}")
            });
        assert!(
            build.status.success(),
            "imported libm f64 native build {explicit_mir} failed:\n{}\n{}",
            String::from_utf8_lossy(&build.stdout),
            String::from_utf8_lossy(&build.stderr)
        );
        assert!(build.stderr.is_empty());
        let native = Command::new(&binary)
            .output()
            .unwrap_or_else(|error| panic!("run imported libm f64 native {explicit_mir}: {error}"));
        assert!(native.status.success());
        assert_eq!(native.stdout, b"8\n");
        assert!(native.stderr.is_empty());
    }
...
#[test]
#[cfg(unix)]
fn canonical_scalar_ffi_f64_imported_libm_mixed_integer_abi_matches_native() {
    if !can_link() {
        eprintln!("SKIP: cc not available");
        return;
    }
    ...
    let dir = project_root().join("target").join(format!(
        "mimi-cli-imported-libm-f64-mixed-{}-{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
    ...
    fs::create_dir_all(&dir).expect("create imported libm mixed f64 directory");
    fs::write(
        dir.join("libm_float.mimi"),
        r#"
pub type Scalar = f64
...
pub func rounded_scaled(value: Scalar, exponent: i32) -> i64 {
    llrint(ldexp(value, exponent))
}
"#,
    )
    .expect("write imported libm mixed f64 module");
    let source = dir.join("main.mimi");
    fs::write(
        &source,
        r#"
use libm_float
func main() -> i64 {
    println(rounded_scaled(1.5, 3));
    0
}
"#,
    )
    .expect("write imported libm mixed f64 entry");
    let binary = dir.join("imported-libm-f64-mixed");
    ...
    for explicit_mir in [false, true] {
        ...
        assert!(
            run.status.success(),
            "imported libm mixed f64 run {explicit_mir} failed:\n{}\n{}",
            String::from_utf8_lossy(&run.stdout),
            String::from_utf8_lossy(&run.stderr)
        );
        ...
        assert!(
            verify.status.success(),
            "imported libm mixed f64 verify {explicit_mir} failed:\n{}\n{}",
            String::from_utf8_lossy(&verify.stdout),
            String::from_utf8_lossy(&verify.stderr)
        );
        assert!(String::from_utf8_lossy(&verify.stdout).contains("No contracts to verify"));
        ...
        assert!(
            build_ir.status.success(),
            "imported libm mixed f64 IR {explicit_mir} failed:\n{}\n{}",
            String::from_utf8_lossy(&build_ir.stdout),
            String::from_utf8_lossy(&build_ir.stderr)
        );
        let ir = String::from_utf8_lossy(&build_ir.stdout);
        assert!(ir.contains("ldexp"));
        assert!(ir.contains("llrint"));
        assert!(ir.contains("double"));
        ...
        let build = build
            ...
            .output()
            .unwrap_or_else(|error| {
                panic!("imported libm mixed f64 native build {explicit_mir}: {error}")
            });
        assert!(
            build.status.success(),
            "imported libm mixed f64 native build {explicit_mir} failed:\n{}\n{}",
            String::from_utf8_lossy(&build.stdout),
            String::from_utf8_lossy(&build.stderr)
        );
        assert!(String::from_utf8_lossy(&build.stderr).is_empty());
        let native = Command::new(&binary).output().unwrap_or_else(|error| {
            panic!("run imported libm mixed f64 native {explicit_mir}: {error}")
        });
        assert!(native.status.success());
        assert_eq!(native.stdout, b"12\n");
        assert!(native.stderr.is_empty());
...
fn canonical_scalar_ffi_f32_imported_module_missing_symbol_preserves_prefix_and_recovers() {
    ...
    let dir = std::env::temp_dir().join(format!(
        "mimi_ffi_cli_f32_imported_missing_{}_{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
    ...
    fs::create_dir_all(&dir).expect("create imported f32 missing-symbol directory");

    let good_c = dir.join("good.c");
    let good_library = dir.join("good.so");
    fs::write(
        &good_c,
        "#include <stdint.h>\nfloat imported_f32(float value) { return value < 0.0f ? -value : value; }\nint64_t imported_f32_check(float value) { return value == 7.5f ? 42 : -1; }\n",
    )
    .expect("write complete imported f32 host fixture");
    let compile_good = Command::new("cc")
        .args(["-shared", "-fPIC", "-O2"])
        .arg(&good_c)
        .arg("-o")
        .arg(&good_library)
        .output()
        .expect("compile complete imported f32 host fixture");
    assert!(
        compile_good.status.success(),
        "{}",
        String::from_utf8_lossy(&compile_good.stderr)
    ...
    .expect("write incomplete imported f32 host fixture");
    let compile_bad = Command::new("cc")
        .args(["-shared", "-fPIC", "-O2"])
        .arg(&bad_c)
        .arg("-o")
        .arg(&bad_library)
        .output()
        .expect("compile incomplete imported f32 host fixture");
    assert!(
        compile_bad.status.success(),
        "{}",
        String::from_utf8_lossy(&compile_bad.stderr)
...
"#,
    ...
    .expect("write imported f32 missing-symbol module");
    let source = dir.join("main.mimi");
    fs::write(
        &source,
        r#"
...
func main() -> i64 {
    println(8 as i64)
    println(check(call(7.5 as Scalar)))
    0
}
"#,
    )
    .expect("write imported f32 missing-symbol entry");
    ...
    assert_eq!(
        missing_symbol_runs[0].stderr, missing_symbol_runs[1].stderr,
        "default and --mir imported f32 missing-symbol diagnostics must match"
    );
    for output in &missing_symbol_runs {
        assert!(!output.status.success());
        assert_eq!(output.stdout, b"8\n");
        ...
        assert!(stderr.contains("imported_f32_check"), "{stderr}");
        assert!(!stderr.contains("canonical route disposition: legacy"));
    }
...
fn canonical_scalar_ffi_f32_imported_module_auxiliary_cli_success_without_contract() {
    ...
    fs::create_dir_all(&dir).expect("create imported f32 auxiliary success directory");
    fs::write(
        dir.join("ffi_types.mimi"),
        r#"
pub type Scalar = f32
...
"#,
    ...
    .expect("write imported f32 auxiliary success module");
    let source = dir.join("main.mimi");
    fs::write(
        &source,
        r#"
...
"#,
    ...
    assert!(
        test.status.success(),
        "imported f32 no-contract test route failed:\n{}\n{}",
        String::from_utf8_lossy(&test.stdout),
        String::from_utf8_lossy(&test.stderr)
    );
    assert!(String::from_utf8_lossy(&test.stdout).contains("✓ test_boundary"));
    ...
    let disasm = Command::new(mimi_bin())
        ...
        .arg(&source)
        .output()
        .expect("spawn imported f32 auxiliary success disasm");
    assert!(
        disasm.status.success(),
        "imported f32 no-contract disasm failed:\n{}\n{}",
        String::from_utf8_lossy(&disasm.stdout),
        String::from_utf8_lossy(&disasm.stderr)
    );
    let disasm_stdout = String::from_utf8_lossy(&disasm.stdout);
    ...
    let mir = Command::new(mimi_bin())
        ...
        .arg(&source)
        .output()
        .expect("spawn imported f32 auxiliary success MIR");
    assert!(
        mir.status.success(),
        "imported f32 no-contract MIR failed:\n{}\n{}",
        String::from_utf8_lossy(&mir.stdout),
        String::from_utf8_lossy(&mir.stderr)
    );
    assert!(String::from_utf8_lossy(&mir.stdout).contains("function:call"));
    ...
    let receipt = Command::new(mimi_bin())
        ...
        .arg(&source)
        .output()
        .expect("spawn imported f32 auxiliary success receipt");
    assert!(
        receipt.status.success(),
        "imported f32 no-contract receipt failed:\n{}\n{}",
        String::from_utf8_lossy(&receipt.stdout),
        String::from_utf8_lossy(&receipt.stderr)
    );
    let repeated_receipt = Command::new(mimi_bin())
        ...
        .arg(&source)
        .output()
        .expect("spawn repeated imported f32 auxiliary success receipt");
    assert!(
        repeated_receipt.status.success(),
        "repeated imported f32 no-contract receipt failed:\n{}\n{}",
        String::from_utf8_lossy(&repeated_receipt.stdout),
        String::from_utf8_lossy(&repeated_receipt.stderr)
    );
    assert_eq!(
        receipt.stdout, repeated_receipt.stdout,
        "imported f32 route manifest must be byte-deterministic"
    );
    let manifest = parse_route_receipt_manifest(&receipt.stdout);
    let manifest_text = String::from_utf8_lossy(&receipt.stdout);
    let checked = checked_route_receipt(&source);
    let manifest_receipt = mimi::core::mir::CanonicalMirRouteReceipt::from_manifest(&manifest_text)
        .expect("imported f32 route manifest must round-trip through the public API");
    assert_eq!(manifest_receipt, checked);
    assert_eq!(manifest.get("mir_digest"), Some(&checked.mir_digest));
    assert_eq!(
        manifest.get("type_desc_digest"),
    ...
    let ffi_digest = manifest.get("ffi_digest").expect("imported f32 FFI digest");
    assert_eq!(ffi_digest.len(), 64);
    assert!(ffi_digest
        .chars()
        .all(|character| character.is_ascii_hexdigit()));
    let root_owners = manifest
        .get("root_owners")
        .expect("imported f32 root owners");
    assert!(root_owners.contains("function:call"));
    assert!(root_owners.contains("function:main"));
    assert!(
        !String::from_utf8_lossy(&receipt.stderr).contains("canonical route disposition: legacy")
...
#[test]
fn canonical_scalar_ffi_f32_imported_module_run_preserves_return_and_order() {
    if !can_link() {
        eprintln!("SKIP: cc not available");
        return;
    }
    let dir = std::env::temp_dir().join(format!(
        "mimi_ffi_cli_f32_imported_run_return_{}_{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
    ...
    fs::create_dir_all(&dir).expect("create imported f32 run return directory");
    let c_path = dir.join("ffi.c");
    let library = dir.join("ffi.so");
    fs::write(
        &c_path,
        "#include <stdint.h>\nstatic int calls;\nfloat imported_f32(float value) { calls += 1; return value; }\nint64_t imported_f32_check(float value) { calls += 1; return value == 7.5f ? 42 : -1; }\nint imported_f32_seen(void) { return calls; }\n",
    )
    .expect("write imported f32 run return C fixture");
    let compile_c = Command::new("cc")
        .args(["-shared", "-fPIC", "-O2"])
        .arg(&c_path)
        .arg("-o")
        .arg(&library)
        .output()
        .expect("compile imported f32 run return C fixture");
    assert!(
        compile_c.status.success(),
        "{}",
        String::from_utf8_lossy(&compile_c.stderr)
...
"#,
    ...
    .expect("write imported f32 run return module");
    let source = dir.join("main.mimi");
    fs::write(
        &source,
        r#"
...
"#,
    ...
    .expect("write imported f32 run return entry");
    ...
    for explicit_mir in [false, true] {
        ...
        let output = command
            ...
            .env("MIMI_FFI_LIB", &library)
            .output()
            .unwrap_or_else(|error| panic!("imported f32 run {explicit_mir}: {error}"));
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(
            output.status.success(),
            "imported f32 run {explicit_mir} failed:\n{stdout}\n{stderr}"
        );
        assert_eq!(stdout, "42\n2\n", "imported f32 run {explicit_mir}");
        assert!(
            stderr.is_empty(),
            "imported f32 run {explicit_mir} emitted diagnostics: {stderr}"
        );
        assert!(!stderr.contains("canonical route disposition: legacy"));
        assert!(!stderr.contains("failed to find canonical MIR FFI symbol"));
    }
...
#[test]
fn canonical_scalar_ffi_f32_contract_imported_module_rejects_before_legacy() {
    let dir = std::env::temp_dir().join(format!(
        "mimi_ffi_cli_f32_contract_imported_{}_{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
...
#[test]
fn canonical_scalar_ffi_cli_cross_call_failure_diagnostics_are_route_stable() {
    if !can_link() {
        return;
    }
    let dir = std::env::temp_dir().join(format!(
        "mimi_ffi_cross_call_failure_cli_{}_{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .expect("system clock")
    ...
    fs::create_dir_all(&dir).expect("create cross-call failure CLI fixture directory");

    let c_path = dir.join("partial.c");
    let partial_library = dir.join("partial.so");
    fs::write(
        &c_path,
        "#include <stdint.h>\nint32_t generated_cli_stable_first(double value) { return value == 1.0 ? 3 : -1; }\n",
    )
    .expect("write partial cross-call host fixture");
    let compile_c = Command::new("cc")
        .args(["-shared", "-fPIC", "-O2"])
        .arg(&c_path)
        .arg("-o")
        .arg(&partial_library)
        .output()
        .expect("compile partial cross-call host fixture");
    assert!(
        compile_c.status.success(),
        "{}",
        String::from_utf8_lossy(&compile_c.stderr)
    ...
    let source = dir.join("cross_call_failure.mimi");
    fs::write(
        &source,
        r#"extern "C" {
    func generated_cli_stable_first(value: f64) -> i32;
    func generated_cli_stable_second(value: i64) -> i32;
}
func main() -> i32 {
    println(8 as i64);
    println(generated_cli_stable_first(1 as i32));
    println(generated_cli_stable_second(20 as i32));
    0
}
"#,
    )
    .expect("write cross-call failure Mimi source");
    ...
    let run = |explicit_mir: bool, library_path: &Path| {
        ...
        command
            ...
            .env("MIMI_FFI_LIB", library_path)
            .output()
            .unwrap_or_else(|error| panic!("cross-call failure run {explicit_mir}: {error}"))
    };
    ...
    assert_eq!(
        missing_library_runs[0].stderr, missing_library_runs[1].stderr,
        "default and --mir missing-library diagnostics must be byte-identical"
    );
    for output in &missing_library_runs {
        assert!(!output.status.success());
        assert_eq!(output.stdout, b"8\n");
    ...
    assert_eq!(
        missing_symbol_runs[0].stderr, missing_symbol_runs[1].stderr,
        "default and --mir missing-symbol diagnostics must be byte-identical"
    );
    for output in &missing_symbol_runs {
        assert!(!output.status.success());
        assert_eq!(output.stdout, b"8\n3\n");
        ...
        assert!(stderr.contains("generated_cli_stable_second"), "{stderr}");
        assert!(!stderr.contains("canonical route disposition: legacy"));
    }
...
#[test]
#[cfg(unix)]
fn canonical_scalar_ffi_imported_default_libc_mixed_verdict_keeps_success_prefix() {
    if !can_link() {
        return;
    }
    let dir = project_root().join("target").join(format!(
        "mimi-cli-imported-default-libc-mixed-verdict-{}-{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .expect("system clock")
    ...
    fs::create_dir_all(&dir).expect("create imported default-libc mixed-verdict fixture");
    fs::write(
        dir.join("libc_guarded.mimi"),
        "extern \"C\" { func labs(value: i64) -> i64 requires: value >= 0; }\npub func first() -> i64 { labs(41 as i64) }\npub func second() -> i64 { labs(-1 as i64) }\n",
    )
    .expect("write imported default-libc mixed-verdict helper");
    let source = dir.join("main.mimi");
    fs::write(
        &source,
        "use libc_guarded\nfunc main() -> i64 { println(first()); println(second()); 0 }\n",
    )
    .expect("write imported default-libc mixed-verdict entry");
    ...
    for explicit_mir in [false, true] {
        ...
        let verify = verify
            ...
            .env_remove("MIMI_FFI_LIB")
            .output()
            .expect("spawn imported default-libc mixed-verdict verification");
        assert!(
            !verify.status.success(),
            "mixed imported default-libc verdict unexpectedly verified (explicit_mir={explicit_mir})"
        );
        let verify_text = format!(
            "{}{}",
            String::from_utf8_lossy(&verify.stdout),
        ...
        assert_eq!(
            ...
            vec!["function:second"],
            "mixed imported default-libc verification must report only the failed wrapper: {verify_text}"
        );
        assert!(
            verify_text.contains("function:first") && verify_text.contains("contract proven"),
            "mixed imported default-libc verification lost the proven wrapper: {verify_text}"
        );
        assert!(
            !verify_text.contains("canonical route disposition: legacy"),
            "mixed imported default-libc verification fell back to legacy: {verify_text}"
        );
        verify_outputs.push(verify);
    }
    assert_eq!(
        ...
        "default and explicit MIR mixed imported verdicts diverged"
    );
    ...
    for explicit_mir in [false, true] {
        ...
        let build = build
            ...
            .env_remove("MIMI_FFI_LIB")
            .output()
            .expect("spawn imported default-libc mixed-verdict build");
        assert!(
            !build.status.success(),
            "mixed imported default-libc verdict unexpectedly built (explicit_mir={explicit_mir})"
        );
        let stderr = String::from_utf8_lossy(&build.stderr);
        assert_eq!(
            disproven_summary(&build),
            vec!["function:second"],
            "mixed imported default-libc build must report only the failed wrapper: {stderr}"
        );
        assert!(
            stderr.contains("FFI contract verification failed"),
            "mixed imported default-libc build lost verification stage: {stderr}"
        );
        assert!(
            !stderr.contains("undefined symbol: labs"),
            "mixed imported default-libc build attempted unresolved linking: {stderr}"
        );
        assert!(
            !stderr.contains("canonical route disposition: legacy"),
            "mixed imported default-libc build fell back to legacy: {stderr}"
        );
        assert!(
            !binary.exists(),
            "failed mixed imported default-libc build left artifact"
        );
        build_outputs.push(build);
    }
    assert_eq!(
        build_outputs[0].stderr, build_outputs[1].stderr,
        "default and explicit MIR mixed imported build diagnostics diverged"
    );
    ...
    for explicit_mir in [false, true] {
        ...
        let run = run
            ...
            .env_remove("MIMI_FFI_LIB")
            .output()
            .expect("spawn imported default-libc mixed-verdict run");
        assert!(
            !run.status.success(),
            "mixed imported default-libc run unexpectedly succeeded (explicit_mir={explicit_mir})"
        );
        assert_eq!(run.stdout, b"41\n", "explicit_mir={explicit_mir}");
        let stderr = String::from_utf8_lossy(&run.stderr);
        assert!(stderr.contains("[E0808]"), "mixed run lost E0808: {stderr}");
        assert!(
            !stderr.contains("canonical route disposition: legacy"),
            "mixed imported default-libc run fell back to legacy: {stderr}"
        );
        ...
        let skipped = skipped
            ...
            .env_remove("MIMI_FFI_LIB")
            .output()
            .expect("spawn imported default-libc mixed-verdict skipped run");
        assert!(
            skipped.status.success(),
            "skipped mixed imported default-libc run failed (explicit_mir={explicit_mir}): {}",
            String::from_utf8_lossy(&skipped.stderr)
        );
...
#[test]
#[cfg(unix)]
fn canonical_scalar_ffi_imported_default_libc_mixed_contract_phases_are_ordered() {
    if !can_link() {
        return;
    }
    let dir = project_root().join("target").join(format!(
        "mimi-cli-imported-default-libc-mixed-contract-phases-{}-{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .expect("system clock")
    ...
    fs::create_dir_all(&dir).expect("create imported default-libc mixed-contract fixture");
    fs::write(
        dir.join("libc_guarded.mimi"),
        "extern \"C\" { func labs(value: i64) -> i64 requires: value <= 0 ensures: value >= 0; }\npub func first() -> i64 { labs(-1 as i64) }\npub func second() -> i64 { labs(41 as i64) }\n",
    )
    .expect("write imported default-libc mixed-contract helper");
    let source = dir.join("main.mimi");
    fs::write(
        &source,
        "use libc_guarded\nfunc main() -> i64 { println(9); println(first()); println(second()); 0 }\n",
    )
    .expect("write imported default-libc mixed-contract entry");

    let phase_summary = |output: &std::process::Output| {
        String::from_utf8_lossy(&output.stdout)
            .lines()
            .chain(String::from_utf8_lossy(&output.stderr).lines())
            .filter_map(|line| {
                let phase = if line.contains("canonical MIR extern requires contract disproven") {
                    "requires"
                } else if line.contains("canonical MIR extern ensures contract disproven") {
                    "ensures"
                } else {
                    return None;
                };
                let function = if line.contains("function:first") {
                    "first"
                } else if line.contains("function:second") {
                    "second"
                } else {
    ...
    let expected = vec![("first", "ensures"), ("second", "requires")];
    let mut verify_outputs = Vec::new();
    for explicit_mir in [false, true] {
        let mut verify = Command::new(mimi_bin());
        verify.current_dir(project_root()).arg("verify");
        ...
        let verify = verify
            ...
            .env_remove("MIMI_FFI_LIB")
            .output()
            .expect("spawn imported default-libc mixed-contract verification");
        assert!(
            !verify.status.success(),
            "mixed imported default-libc contract unexpectedly verified (explicit_mir={explicit_mir})"
        );
        let verify_text = format!(
            "{}{}",
            String::from_utf8_lossy(&verify.stdout),
        ...
        assert_eq!(
            ...
            expected,
            "mixed imported default-libc contract phases changed: {verify_text}"
        );
        assert!(
            !verify_text.contains("canonical route disposition: legacy"),
            "mixed imported default-libc contract verification fell back to legacy: {verify_text}"
        );
        verify_outputs.push(verify);
    }
    assert_eq!(
        phase_summary(&verify_outputs[0]),
        phase_summary(&verify_outputs[1]),
        "default and explicit MIR mixed imported contract phases diverged"
    );

    let mut build_outputs = Vec::new();
    for explicit_mir in [false, true] {
        let binary = dir.join(if explicit_mir {
            "mixed-contract-phases-mir"
        } else {
            "mixed-contract-phases-default"
        });
        let mut build = Command::new(mimi_bin());
        build.current_dir(project_root()).arg("build");
        if explicit_mir {
        ...
        let build = build
            ...
            .env_remove("MIMI_FFI_LIB")
            .output()
            .expect("spawn imported default-libc mixed-contract build");
        assert!(
            !build.status.success(),
            "mixed imported default-libc contract unexpectedly built (explicit_mir={explicit_mir})"
        );
        let build_text = format!(
            "{}{}",
            String::from_utf8_lossy(&build.stdout),
        ...
        assert_eq!(
            phase_summary(&build),
            expected,
            "mixed imported default-libc build phases changed: {build_text}"
        );
        assert!(
            build_text.contains("FFI contract verification failed"),
            "mixed imported default-libc build lost verification stage: {build_text}"
        );
        assert!(
            !build_text.contains("undefined symbol: labs"),
            "mixed imported default-libc build attempted unresolved linking: {build_text}"
        );
        assert!(
            !build_text.contains("canonical route disposition: legacy"),
            "mixed imported default-libc build fell back to legacy: {build_text}"
        );
        assert!(
            !binary.exists(),
            "failed mixed imported default-libc build left artifact"
    ...
    assert_eq!(
        build_outputs[0].stderr, build_outputs[1].stderr,
        "default and explicit MIR mixed imported contract build diagnostics diverged"
    );
    ...
    for explicit_mir in [false, true] {
        ...
        let run = run
            ...
            .env_remove("MIMI_FFI_LIB")
            .output()
            .expect("spawn imported default-libc mixed-contract run");
        assert!(
            !run.status.success(),
            "mixed imported default-libc contract run unexpectedly succeeded (explicit_mir={explicit_mir})"
        );
        assert_eq!(run.stdout, b"9\n", "explicit_mir={explicit_mir}");
        let stderr = String::from_utf8_lossy(&run.stderr);
        assert!(
            stderr.contains("[E0808]"),
            "mixed contract run lost E0808: {stderr}"
        );
        assert!(
            !stderr.contains("canonical route disposition: legacy"),
            "mixed imported default-libc contract run fell back to legacy: {stderr}"
        );
        ...
        let skipped = skipped
            ...
            .env_remove("MIMI_FFI_LIB")
            .output()
            .expect("spawn imported default-libc mixed-contract skipped run");
        assert!(
            skipped.status.success(),
            "skipped mixed imported default-libc contract run failed (explicit_mir={explicit_mir}): {}",
            String::from_utf8_lossy(&skipped.stderr)
        );
        assert_eq!(skipped.stdout, b"9\n1\n41\n", "explicit_mir={explicit_mir}");
    }
...
#[test]
#[cfg(unix)]
fn canonical_scalar_ffi_imported_result_ensures_keep_static_and_runtime_boundaries() {
    if !can_link() {
        return;
    }
    let dir = project_root().join("target").join(format!(
        "mimi-cli-imported-result-ensures-{}-{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .expect("system clock")
    ...
    fs::create_dir_all(&dir).expect("create imported result-ensures fixture");
    let c_path = dir.join("ffi.c");
    let library = dir.join("ffi.so");
    fs::write(
        &c_path,
        "#include <stdint.h>\nint64_t mir_ffi_import_result(int64_t value) { return value == 7 ? value : value + 1; }\n",
    )
    .expect("write imported result-ensures C fixture");
    let compile_c = Command::new("cc")
        .args(["-shared", "-fPIC", "-O2"])
        .arg(&c_path)
        .arg("-o")
        .arg(&library)
        .output()
        .expect("compile imported result-ensures C fixture");
    assert!(
        compile_c.status.success(),
        "{}",
        String::from_utf8_lossy(&compile_c.stderr)
    ...
    fs::write(
        dir.join("ffi_types.mimi"),
        "extern \"C\" { func mir_ffi_import_result(value: i64) -> i64 ensures: result == value; }\npub func first() -> i64 { mir_ffi_import_result(7 as i64) }\npub func second() -> i64 { mir_ffi_import_result(8 as i64) }\n",
    )
    .expect("write imported result-ensures module");
    let source = dir.join("main.mimi");
    fs::write(
        &source,
        "use ffi_types\nfunc main() -> i64 { println(9); println(first()); println(second()); 0 }\n",
    )
    .expect("write imported result-ensures entry");

    let ensures_summary = |output: &std::process::Output| {
        String::from_utf8_lossy(&output.stdout)
            .lines()
            .chain(String::from_utf8_lossy(&output.stderr).lines())
            .filter(|line| line.contains("canonical MIR extern ensures contract disproven"))
    ...
    for explicit_mir in [false, true] {
        ...
        let verify = verify
            ...
            .env("MIMI_FFI_LIB", &library)
            .output()
            .expect("spawn imported result-ensures verification");
        assert!(
            !verify.status.success(),
            "imported result-ensures verification unexpectedly succeeded (explicit_mir={explicit_mir})"
        );
        let verify_text = format!(
            "{}{}",
            String::from_utf8_lossy(&verify.stdout),
        ...
        assert_eq!(
            ensures_summary(&verify),
            expected,
            "imported result-ensures verification changed call-site order: {verify_text}"
        );
        assert!(
            !verify_text.contains("canonical route disposition: legacy"),
            "imported result-ensures verification fell back to legacy: {verify_text}"
        );
        verify_outputs.push(verify);
    }
    assert_eq!(
        ensures_summary(&verify_outputs[0]),
        ensures_summary(&verify_outputs[1]),
        "default and explicit MIR imported result-ensures verification diverged"
    );
    ...
    for explicit_mir in [false, true] {
        ...
        let build = build
            ...
            .env("MIMI_FFI_LIB", &library)
            .output()
            .expect("spawn imported result-ensures build");
        assert!(
            !build.status.success(),
            "imported result-ensures build unexpectedly succeeded (explicit_mir={explicit_mir})"
        );
        let build_text = format!(
            "{}{}",
            String::from_utf8_lossy(&build.stdout),
        ...
        assert_eq!(
            ensures_summary(&build),
            expected,
            "imported result-ensures build changed call-site order: {build_text}"
        );
        assert!(
            build_text.contains("FFI contract verification failed"),
            "imported result-ensures build lost verification phase: {build_text}"
        );
        assert!(
            !build_text.contains("undefined symbol: mir_ffi_import_result"),
            "imported result-ensures build attempted unresolved linking: {build_text}"
        );
        assert!(
            !build_text.contains("canonical route disposition: legacy"),
            "imported result-ensures build fell back to legacy: {build_text}"
        );
        assert!(
            !binary.exists(),
            "failed imported result-ensures build left artifact"
        );
        build_outputs.push(build);
    }
    assert_eq!(
        build_outputs[0].stderr, build_outputs[1].stderr,
        "default and explicit MIR imported result-ensures build diagnostics diverged"
    );
    ...
    for explicit_mir in [false, true] {
        ...
        let run = run
            ...
            .env("MIMI_FFI_LIB", &library)
            .output()
            .expect("spawn imported result-ensures run");
        assert!(
            !run.status.success(),
            "imported result-ensures run unexpectedly succeeded (explicit_mir={explicit_mir})"
        );
        assert_eq!(run.stdout, b"9\n7\n", "explicit_mir={explicit_mir}");
        let stderr = String::from_utf8_lossy(&run.stderr);
        assert!(
            stderr.contains("[E0808] FFI postcondition failed"),
            "imported result-ensures run lost postcondition failure: {stderr}"
        );
        assert!(
            !stderr.contains("canonical route disposition: legacy"),
            "imported result-ensures run fell back to legacy: {stderr}"
        );
        ...
        let skipped = skipped
            ...
            .env("MIMI_FFI_LIB", &library)
            .output()
            .expect("spawn imported result-ensures skipped run");
        assert!(
            skipped.status.success(),
            "skipped imported result-ensures run failed (explicit_mir={explicit_mir}): {}",
            String::from_utf8_lossy(&skipped.stderr)
        );
        assert_eq!(skipped.stdout, b"9\n7\n9\n", "explicit_mir={explicit_mir}");
    }
...
#[test]
#[cfg(unix)]
fn canonical_scalar_ffi_imported_bool_result_ensures_match_cli_consumers() {
    if !can_link() {
        return;
    }
    let dir = project_root().join("target").join(format!(
        "mimi-cli-imported-bool-result-ensures-{}-{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .expect("system clock")
    ...
    fs::create_dir_all(&dir).expect("create imported bool result-ensures fixture");
    let c_path = dir.join("ffi.c");
    let library = dir.join("ffi.so");
    fs::write(
        &c_path,
        "#include <stdbool.h>\nbool mir_ffi_import_bool(bool value) { return value; }\n",
    )
    .expect("write imported bool result-ensures C fixture");
    let compile_c = Command::new("cc")
        .args(["-shared", "-fPIC", "-O2"])
        .arg(&c_path)
        .arg("-o")
        .arg(&library)
        .output()
        .expect("compile imported bool result-ensures C fixture");
    assert!(
        compile_c.status.success(),
        "{}",
        String::from_utf8_lossy(&compile_c.stderr)
    ...
    fs::write(
        dir.join("ffi_types.mimi"),
        "extern \"C\" { func mir_ffi_import_bool(value: bool) -> bool ensures: result == value; }\npub func first() -> bool { mir_ffi_import_bool(false) }\npub func second() -> bool { mir_ffi_import_bool(true) }\n",
    )
    .expect("write imported bool result-ensures module");
    let source = dir.join("main.mimi");
    fs::write(
        &source,
        "use ffi_types\nfunc main() -> i64 { println(first()); println(second()); 0 }\n",
    )
    .expect("write imported bool result-ensures entry");
    ...
    for explicit_mir in [false, true] {
        ...
        let verify = verify
            ...
            .env("MIMI_FFI_LIB", &library)
            .output()
            .expect("spawn imported bool result-ensures verification");
        assert!(
            !verify.status.success(),
            "imported bool result-ensures verification unexpectedly succeeded (explicit_mir={explicit_mir})"
        );
        let verify_text = format!(
            "{}{}",
            String::from_utf8_lossy(&verify.stdout),
        ...
        assert_eq!(
            ensures_summary(&verify),
            expected,
            "imported bool result-ensures verification changed call-site order: {verify_text}"
        );
        assert!(
            !verify_text.contains("canonical route disposition: legacy"),
            "imported bool result-ensures verification fell back to legacy: {verify_text}"
        );
        verify_outputs.push(verify);
    }
    assert_eq!(
        ensures_summary(&verify_outputs[0]),
        ensures_summary(&verify_outputs[1]),
        "default and explicit MIR imported bool result-ensures verification diverged"
    );

    let mut build_outputs = Vec::new();
    for explicit_mir in [false, true] {
        let binary = dir.join(if explicit_mir {
            "bool-result-ensures-mir"
        } else {
            "bool-result-ensures-default"
        });
        let mut build = Command::new(mimi_bin());
        build.current_dir(project_root()).arg("build");
        if explicit_mir {
        ...
        let build = build
            ...
            .env("MIMI_FFI_LIB", &library)
            .output()
            .expect("spawn imported bool result-ensures build");
        assert!(
            !build.status.success(),
            "imported bool result-ensures build unexpectedly succeeded (explicit_mir={explicit_mir})"
        );
        let build_text = format!(
            "{}{}",
            String::from_utf8_lossy(&build.stdout),
        ...
        assert_eq!(
            ensures_summary(&build),
            expected,
            "imported bool result-ensures build changed call-site order: {build_text}"
        );
        assert!(
            build_text.contains("FFI contract verification failed"),
            "imported bool result-ensures build lost verification phase: {build_text}"
        );
        assert!(
            !build_text.contains("undefined symbol: mir_ffi_import_bool"),
            "imported bool result-ensures build attempted unresolved linking: {build_text}"
        );
        assert!(
            !build_text.contains("canonical route disposition: legacy"),
            "imported bool result-ensures build fell back to legacy: {build_text}"
        );
        assert!(
            !binary.exists(),
            "failed imported bool result-ensures build left artifact"
        );
        build_outputs.push(build);
    }
    assert_eq!(
        build_outputs[0].stderr, build_outputs[1].stderr,
        "default and explicit MIR imported bool result-ensures build diagnostics diverged"
    );
    ...
    for explicit_mir in [false, true] {
        ...
        let run = run
            ...
            .env("MIMI_FFI_LIB", &library)
            .output()
            .expect("spawn imported bool result-ensures run");
        assert!(
            run.status.success(),
            "imported bool result-ensures run failed (explicit_mir={explicit_mir}): {}",
            String::from_utf8_lossy(&run.stderr)
        );
        assert_eq!(run.stdout, b"false\ntrue\n", "explicit_mir={explicit_mir}");
        assert!(
            !String::from_utf8_lossy(&run.stderr).contains("canonical route disposition: legacy"),
            "imported bool result-ensures run fell back to legacy: {}",
            String::from_utf8_lossy(&run.stderr)
        );
        ...
        let skipped = skipped
            ...
            .env("MIMI_FFI_LIB", &library)
            .output()
            .expect("spawn imported bool result-ensures skipped run");
        assert!(
            skipped.status.success(),
            "skipped imported bool result-ensures run failed (explicit_mir={explicit_mir}): {}",
            String::from_utf8_lossy(&skipped.stderr)
        );
        assert_eq!(
            skipped.stdout, b"false\ntrue\n",
            "explicit_mir={explicit_mir}"
        );
    }
    fs::remove_dir_all(&dir).ok();
...
fn canonical_flow_failure_match_returns_source_on_default_mir_route() {
    ...
    let mir_stdout = String::from_utf8_lossy(&mir_dump.stdout);
    assert!(mir_stdout.contains("recoverable_boundary"));
    assert!(mir_stdout.contains("nested=MirTupleProjectionContract"));
...
#[test]
fn canonical_default_generic_option_f32_unwrap_matches_all_consumers() {
    let fixture = project_root()
        .join("tests")
        .join("fixtures")
        .join("mir_native_generic_option_unwrap_f32.mimi");
    ...
    let default_run = Command::new(mimi_bin())
        ...
        .arg(&fixture)
        .output()
        .expect("failed to spawn default generic Option<f32> projection run");
    assert_eq!(default_run.status.code(), Some(42));
    ...
    let explicit_mir_run = Command::new(mimi_bin())
        ...
        .arg("--mir")
        .output()
        .expect("failed to spawn explicit MIR generic Option<f32> projection run");
    assert_eq!(explicit_mir_run.status.code(), Some(42));
    ...
    let verify = Command::new(mimi_bin())
        ...
        .arg(&fixture)
        .output()
        .expect("failed to spawn default generic Option<f32> projection verifier");
    assert!(
        verify.status.success(),
        "default generic Option<f32> projection verifier failed:\n{}",
        String::from_utf8_lossy(&verify.stderr)
    );
    assert!(String::from_utf8_lossy(&verify.stdout).contains("canonical MIR"));

    let binary = std::env::temp_dir().join(format!(
        "mimi-default-generic-option-f32-{}",
        std::process::id()
    ));
    let build = Command::new(mimi_bin())
        .current_dir(project_root())
        ...
        .arg(&binary)
        .output()
        .expect("failed to spawn default generic Option<f32> projection native build");
    assert!(
        build.status.success(),
        "default generic Option<f32> projection native build failed:\n{}",
        String::from_utf8_lossy(&build.stderr)
    );
    let native_run = Command::new(&binary)
        .output()
        .expect("failed to execute default generic Option<f32> projection native binary");
    let _ = fs::remove_file(&binary);
    assert_eq!(native_run.status.code(), Some(42));
}
...
#[test]
fn canonical_mir_f64_cross_function_print_routes_canonical_and_matches_native() {
    // R6-1070: the one-edge f64 print closure routes canonical on the
    // default entry — a helper printing its own f64 parameter, a helper
    // returning an f64 literal printed through the call result, and a
    // helper whose seeded-parameter arithmetic is the printed root all
    // classify complete with no legacy disposition, and the native MIR
    // binary agrees with the default backend on every shape.
    let fixture = project_root()
        .join("tests")
        .join("fixtures")
        .join("mir_native_f64_cross_function_print.mimi");
    let expected = "2.5\n1.5\n3\n";

    let binary = std::env::temp_dir().join(format!(
        "mimi-canonical-native-f64-cross-function-{}",
        std::process::id()
    ));
    let build = Command::new(mimi_bin())
        .current_dir(project_root())
        ...
        .arg(&binary)
        .output()
        .expect("failed to spawn cross-function f64 print native build");
    assert!(
        build.status.success(),
        "the admitted cross-function f64 print face must build natively:\n{}",
        String::from_utf8_lossy(&build.stderr)
    );
    let native = Command::new(&binary).output().expect("run native binary");
    let _ = fs::remove_file(&binary);
    ...
    let default = Command::new(mimi_bin())
        ...
        .arg(&fixture)
        .output()
        .expect("failed to run cross-function f64 print fixture on the default backend");
    assert!(default.status.success());
    assert_eq!(
        String::from_utf8_lossy(&default.stdout),
        native_stdout,
        "default and native MIR must agree on the cross-function print face"
    );
    let default_stderr = String::from_utf8_lossy(&default.stderr).to_string();
    assert!(
        !default_stderr.contains("canonical route disposition: legacy"),
        "the admitted cross-function print face must not fall back to legacy:\n{default_stderr}"
    );
}
...
#[test]
fn canonical_mir_f64_compare_call_routes_canonical_and_matches_native() {
    // R6-1072: the comparison operand domain admits face-closure calls —
    // a direct comparison, arithmetic over the call result, and a negate
    // over it all route canonical on the default entry, with the native
    // MIR binary agreeing byte-for-byte.  Comparing a call requires
    // calling it, so the callee is by construction one face edge from the
    // print receipt.
    let fixture = project_root()
        .join("tests")
        .join("fixtures")
        .join("mir_native_f64_compare_call.mimi");
    let expected = "0.5\n7\n8\n11\n";

    let binary = std::env::temp_dir().join(format!(
        "mimi-canonical-native-f64-compare-call-{}",
        std::process::id()
    ));
    let build = Command::new(mimi_bin())
        .current_dir(project_root())
        ...
        .arg(&binary)
        .output()
        .expect("failed to spawn f64 compare-call native build");
    assert!(
        build.status.success(),
        "the admitted f64 compare-call face must build natively:\n{}",
        String::from_utf8_lossy(&build.stderr)
    );
    let native = Command::new(&binary).output().expect("run native binary");
    let _ = fs::remove_file(&binary);
    ...
    let default = Command::new(mimi_bin())
        ...
        .arg(&fixture)
        .output()
        .expect("failed to run f64 compare-call fixture on the default backend");
    assert!(default.status.success());
    assert_eq!(
        String::from_utf8_lossy(&default.stdout),
        native_stdout,
        "default and native MIR must agree on the compare-call face"
    );
    let default_stderr = String::from_utf8_lossy(&default.stderr).to_string();
    assert!(
        !default_stderr.contains("canonical route disposition: legacy"),
        "the admitted compare-call face must not fall back to legacy:\n{default_stderr}"
    );
}
...
#[test]
fn canonical_mir_owned_string_nested_diamond_routes_canonical_and_verifies() {
    // R6-1087: the nested-diamond face — the outer then arm is itself
    // Branch-terminated, which the R6-1086 join-stopped sub-graph test
    // held, so the outer-else path leaked (valgrind: 3 bytes in 1 block).
    // Reachability-based placement drops the value at both sibling heads
    // (outer-else and inner-else, mutually exclusive arms), the native
    // binary runs clean, the default backend agrees byte-for-byte, and
    // the ensures contract verifies on the single canonical engine.
    let fixture = project_root()
        .join("tests")
        .join("fixtures")
        .join("mir_native_owned_string_nested_diamond.mimi");
    let expected = "3\n";

    let binary = std::env::temp_dir().join(format!(
        "mimi-canonical-native-owned-string-nested-diamond-{}",
        std::process::id()
    ));
    let build = Command::new(mimi_bin())
        .current_dir(project_root())
        ...
        .arg(&binary)
        .output()
        .expect("failed to spawn owned-string nested-diamond native build");
    assert!(
        build.status.success(),
        "the admitted owned-String nested-diamond face must build natively:\n{}",
        String::from_utf8_lossy(&build.stderr)
    );
    let native = Command::new(&binary).output().expect("run native binary");
    let _ = fs::remove_file(&binary);
    ...
    let default = Command::new(mimi_bin())
        ...
        .arg(&fixture)
        .output()
        .expect("failed to run owned-string nested-diamond fixture on the default backend");
    assert!(default.status.success());
    assert_eq!(
        String::from_utf8_lossy(&default.stdout),
        native_stdout,
        "default and native MIR must agree on the owned-String nested-diamond face"
    );
    let default_stderr = String::from_utf8_lossy(&default.stderr).to_string();
    assert!(
        !default_stderr.contains("canonical route disposition: legacy"),
        "the admitted owned-String nested-diamond face must not fall back to legacy:\n{default_stderr}"
    );
    ...
    let verify = Command::new(mimi_bin())
        ...
        .arg("--mir")
        .output()
        .expect("owned-string nested-diamond verify");
    assert!(
        verify.status.success(),
        "the owned-String nested-diamond contract must verify:\n{}\n{}",
        String::from_utf8_lossy(&verify.stdout),
        String::from_utf8_lossy(&verify.stderr)
    );
    let verify_stdout = String::from_utf8_lossy(&verify.stdout);
...
#[test]
fn canonical_mir_option_i32_match_routes_canonical_and_verifies() {
    // R6-1089: the Copy Option<i32> match face — `match` lowers to a plain
    // `Switch` with `Variant` arms over a cloned Copy scrutinee, and the
    // copy-option island now admits it as a first-class candidate: the
    // default route selects the canonical engine with no legacy fallback,
    // the native MIR binary agrees byte-for-byte, and the ensures contract
    // verifies on that engine.
    let fixture = project_root()
        .join("tests")
        .join("fixtures")
        .join("mir_native_option_i32_match.mimi");
    let expected = "";

    let binary = std::env::temp_dir().join(format!(
        "mimi-canonical-native-option-i32-match-{}",
        std::process::id()
    ));
    let build = Command::new(mimi_bin())
        .current_dir(project_root())
        ...
        .arg(&binary)
        .output()
        .expect("failed to spawn copy-option match native build");
    assert!(
        build.status.success(),
        "the admitted Copy Option<i32> match face must build natively:\n{}",
        String::from_utf8_lossy(&build.stderr)
    );
    let native = Command::new(&binary).output().expect("run native binary");
    let _ = fs::remove_file(&binary);
    ...
    let default = Command::new(mimi_bin())
        ...
        .arg(&fixture)
        .output()
        .expect("failed to run copy-option match fixture on the default backend");
    assert!(default.status.success());
    assert_eq!(
        String::from_utf8_lossy(&default.stdout),
        native_stdout,
        "default and native MIR must agree on the Copy Option<i32> match face"
    );
    let default_stderr = String::from_utf8_lossy(&default.stderr).to_string();
    assert!(
        !default_stderr.contains("canonical route disposition: legacy"),
        "the admitted Copy Option<i32> match face must not fall back to legacy:\n{default_stderr}"
    );
    ...
    let verify = Command::new(mimi_bin())
        ...
        .arg("--mir")
        .output()
        .expect("copy-option match verify");
    assert!(
        verify.status.success(),
        "the Copy Option<i32> match contract must verify:\n{}\n{}",
        String::from_utf8_lossy(&verify.stdout),
        String::from_utf8_lossy(&verify.stderr)
    );
    let verify_stdout = String::from_utf8_lossy(&verify.stdout);
...
#[test]
fn canonical_mir_result_i32_match_routes_canonical_and_verifies() {
    // R6-1090: the Copy Result<i32, i32> match face — the mirror of the
    // R6-1089 Copy Option admission.  `match` lowers to a plain `Switch`
    // with `Ok`/`Err` `Variant` arms over a cloned Copy scrutinee, and the
    // copy-result island now admits it as a first-class candidate: the
    // default route selects the canonical engine with no legacy fallback,
    // the native MIR binary agrees byte-for-byte, and the ensures contract
    // verifies on that engine.
    let fixture = project_root()
        .join("tests")
        .join("fixtures")
        .join("mir_native_result_i32_match.mimi");
    let expected = "";

    let binary = std::env::temp_dir().join(format!(
        "mimi-canonical-native-result-i32-match-{}",
        std::process::id()
    ));
    let build = Command::new(mimi_bin())
        .current_dir(project_root())
        ...
        .arg(&binary)
        .output()
        .expect("failed to spawn copy-result match native build");
    assert!(
        build.status.success(),
        "the admitted Copy Result<i32, i32> match face must build natively:\n{}",
        String::from_utf8_lossy(&build.stderr)
    );
    let native = Command::new(&binary).output().expect("run native binary");
    let _ = fs::remove_file(&binary);
    ...
    let default = Command::new(mimi_bin())
        ...
        .arg(&fixture)
        .output()
        .expect("failed to run copy-result match fixture on the default backend");
    assert!(default.status.success());
    assert_eq!(
        String::from_utf8_lossy(&default.stdout),
        native_stdout,
        "default and native MIR must agree on the Copy Result<i32, i32> match face"
    );
    let default_stderr = String::from_utf8_lossy(&default.stderr).to_string();
    assert!(
        !default_stderr.contains("canonical route disposition: legacy"),
        "the admitted Copy Result<i32, i32> match face must not fall back to legacy:\n{default_stderr}"
    );
    ...
    let verify = Command::new(mimi_bin())
        ...
        .arg("--mir")
        .output()
        .expect("copy-result match verify");
    assert!(
        verify.status.success(),
        "the Copy Result<i32, i32> match contract must verify:\n{}\n{}",
        String::from_utf8_lossy(&verify.stdout),
        String::from_utf8_lossy(&verify.stderr)
    );
    let verify_stdout = String::from_utf8_lossy(&verify.stdout);
...
// R6-1057 face opening, mirroring the R6-1054 bind pin: a Copy f64 local
// target reassigned from a float literal lowers as Const(FloatBits) -> Move
// (fresh slot materialization; Copy scalars never need drop glue), routes
// canonical on the default entry, and is differential-pinned across the
// explicit `--mir` run and the native `build --mir` binary.
#[test]
fn canonical_mir_float_assign_routes_canonical_across_consumers() {
    let source = project_root()
        .join("tests")
        .join("fixtures")
        .join("mir_float_assign_face.mimi");
    ...
    assert!(
        run.status.success(),
        "float assign program must execute: {stderr}"
    );
    assert_eq!(run.stdout, b"1.5\n");
    assert!(
        !stderr.contains("canonical route disposition: legacy"),
        "float assign program must route canonical on the default entry: {stderr}"
    );
    ...
    assert_eq!(mir_run.stdout, b"1.5\n");

    let native = project_root()
        .join("target")
        .join(format!("mir_float_assign_native_{}", std::process::id()));
    let build = Command::new(mimi_bin())
        .current_dir(project_root())
        .arg("build")
        .arg(&source)
    ...
    let native_run = Command::new(&native).output().unwrap();
    assert!(native_run.status.success());
    assert_eq!(native_run.stdout, b"1.5\n");
    let _ = fs::remove_file(&native);
}

// R6-1059 face opening: a second-hand float assign (`x = y` with a plain
// local read as RHS) routes canonical on the default entry, the routed
// `mimi verify` entry proves the ensures contract on the MIR path (the
// legacy-path verifier keeps floats uninterpreted and failed this exact
// program before the flip), and the native `build --mir` binary agrees.
#[test]
fn canonical_mir_float_second_hand_assign_routes_canonical_with_proof() {
    let source = project_root()
        .join("tests")
        .join("fixtures")
        .join("mir_float_second_hand_assign.mimi");
    ...
    assert!(
        run.status.success(),
        "second-hand float assign program must execute: {stderr}"
    );
    assert_eq!(run.stdout, b"1.5\n");
    assert!(
        !stderr.contains("canonical route disposition: legacy"),
        "second-hand float assign must route canonical on the default entry: {stderr}"
    );
    ...
    let native = project_root().join("target").join(format!(
        "mir_float_second_hand_assign_native_{}",
        std::process::id()
    ));
    let build = Command::new(mimi_bin())
        .current_dir(project_root())
...
// R6-1062 face opening: an integer literal widening into an F64 assign
// target (`x = 2`) keeps the target in the symbolic Float domain, so the
// arithmetic that follows routes canonical on the default entry, the routed
// `mimi verify` entry proves the ensures contract through the exactly-widened
// known constant, and the native `build --mir` binary agrees byte for byte.
#[test]
fn canonical_mir_int_widen_arithmetic_routes_canonical_with_proof() {
    let source = project_root()
        .join("tests")
        .join("fixtures")
        .join("mir_int_widen_arithmetic_face.mimi");
    let expected = b"2\n3\n";
    ...
    assert!(
        run.status.success(),
        "int widen arithmetic program must execute: {stderr}"
    );
    assert_eq!(run.stdout, expected);
    assert!(
        !stderr.contains("canonical route disposition: legacy"),
        "int widen arithmetic program must route canonical on the default entry: {stderr}"
    );
    ...
    let native = project_root()
        .join("target")
        .join(format!("mir_int_widen_native_{}", std::process::id()));
    let build = Command::new(mimi_bin())
        .current_dir(project_root())
        .arg("build")
        .arg(&source)
...
// R6-1065: an f64 contract literal face.  `result == 1.5` and
// `result >= 1.0` lower to `MirContractExpr::Float(bits)`; the direct
// `mimi verify --mir` entry proves both on a constant-returning body, the
// default run executes byte-identically across entries while keeping its
// explicit compatibility disposition (the call-result print root is still
// a registered mixed floor), and the native `build --mir` binary agrees.
#[test]
fn canonical_mir_float_contract_literal_verifies_on_mir_entry() {
    let source = project_root()
        .join("tests")
        .join("fixtures")
        .join("mir_float_contract_literal_face.mimi");
    let expected = b"1.5\n";
    ...
    assert!(
        run.status.success(),
        "float literal contract fixture must execute: {stderr}"
    );
    assert_eq!(run.stdout, expected);
    assert!(
        stderr.contains(
        ...
        "float literal fixture must keep the compatibility default route: {stderr}"
    );
    ...
    assert!(
        verify_out.contains("2/2 verified"),
        "both literal obligations must verify on the MIR entry: {verify_out}"
    );
    ...
    let native = project_root().join("target").join(format!(
        "mir_float_contract_literal_native_{}",
        std::process::id()
    ));
    let build = Command::new(mimi_bin())
        .current_dir(project_root())
...
// R6-1066: an IEEE negate face.  The runtime symbolic interpreter admits a
// float Unary negate (exact sign flip, no MIN-edge definedness) and the
// contract domain admits `-(x)`; the body/contract round-trip proves on
// the direct `mimi verify --mir` entry, the default run keeps its
// explicit compatibility disposition, and all three consumers agree on
// the printed `-1.5`.
#[test]
fn canonical_mir_float_contract_negate_verifies_on_mir_entry() {
    let source = project_root()
        .join("tests")
        .join("fixtures")
        .join("mir_float_contract_negate_face.mimi");
    let expected = b"-1.5\n";
    ...
    assert!(
        run.status.success(),
        "float negate fixture must execute: {stderr}"
    );
    assert_eq!(run.stdout, expected);
    assert!(
        stderr.contains(
        ...
        "float negate fixture must keep the compatibility default route: {stderr}"
    );
    ...
    assert!(
        verify_out.contains("1/1 verified"),
        "the negate round-trip contract must verify on the MIR entry: {verify_out}"
    );
    ...
    let native = project_root().join("target").join(format!(
        "mir_float_contract_negate_native_{}",
        std::process::id()
    ));
    let build = Command::new(mimi_bin())
        .current_dir(project_root())
