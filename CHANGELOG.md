...
- **架构重构（0.1.0 收尾）**：
  - `src/runtime/mod.rs` 拆分（24105→18152 行，14 个模块）：regex/lexer/crypto/fs/binary_io/future/ffi_test/concurrency/actor/quote/net/shadow_mte/capability/env 抽出，机械拆分不改语义，419 个 `#[no_mangle]` 符号全导出、4053 测试绿。硬共享簇（map/set/list/string/json ~180 extern fn）函数交错且互引，作为耦合核心保留 mod.rs。
  - `src/core/resolved.rs` 拆分（12702→8551 行）：目录化为 resolved/mod.rs，`#[cfg(test)] mod tests`（4152 行）分离到 resolved/tests.rs。identity/catalog/walk 生产代码边界模糊且重度耦合，作为耦合核心保留 mod.rs。
...
## [0.1.0] — 基线稳定（原 v0.31.0–v0.31.7 开发周期）
...
- 修正两个 verifier 测试夹具（续）：`verify_invariant_preserve_assign_degrades` 的参数 `x` 未声明 `mut` 却被赋值（checker 拒绝），改为 `mut x` 以保留「循环内赋值 invariant 自由变量使状态降级」的测试意图；`verify_shared_param_field_scalar_contract` 改为锁定 verifier 对 shared 参数合约的 fail-closed 拒绝（Z3 无法建模 shared 堆状态，shared scalar 验证为后续增强），杜绝静默 unsound「Verified」。
- 修正两个 verifier 测试夹具：`verify_comprehension_in_body_not_crash` 使用非法的单参 `range(n)`（range 在 interp/codegen/inference 均要求 2 参），改为 `range(0, n)`；`verify_match_nonexhaustive_no_false_positive` 改为接受检查器对无 wildcard 的 i32 match 的硬拒绝（verify_source 返回 Err）作为「无误报」outcome（比 Failed/Unknown 更强的保证），不再 expect Ok。
- 修复 verifier 合约语言中 `char_at(s,i)` / `starts_with(s,p)` / `ends_with(s,p)` 函数式拼写无法解析（TOOL-RESOLUTION-001 回归）：verifier Z3 层与 inference 方法位已支持这些裸名，但（1）inference 函数调用分发（infer/call/simple.rs）缺这些内建，落入 "undefined function"；（2）`is_builtin_callable`（core/builtins.rs）缺裸名，call-site 目录将其归类 `ResolvedCallKind::Unknown`，typed-body lowering fail-closed（"closed Unknown call target"）。两处补齐后合约中的裸名 builtin 正确解析为 Builtin 调用。恢复 `verify_string_char_at_contract` / `verify_string_starts_ends_with`。
...
- 修复 codegen list 变异与 `view`/`mutate` borrow 路径三处 `Located` 包裹漏网（同为 v0.31.1 Span/Origin 系统性根因，L1+L3）：
  - `compile_call` 的 `push`/`pop` alloca-swap 改为对 `args[0].unlocated()` 匹配。此前 `match &args[0]` 直接匹配 `Expr::Ident/Field`，`Located(..)` 包裹的列表实参落入 `_ => {}`，swap 不触发，`push` 变异被丢弃的临时 alloca 而非调用方列表，下一次 push 读已释放缓冲 → double free。恢复 `dual_from_json_list_push_i64` / `dual_from_json_list_push_then_len`。
  - `prepare_borrowed_user_args` 的实参匹配改为对 `arg_exprs[index].unlocated()` 匹配。此前 `mutate List` 参数落入 rvalue 分支、收到指向临时副本的指针，callee 内 `push` 无法到达调用方列表（len 停滞 → 调用方后续索引读 FFI 越界）。恢复 `mutate_list_push_allowed`。
  - `list_is_already_indirect` 改为对 `param.ty.unlocated()` 匹配。此前 `Type::Located(Name("List"))` 不匹配 `Name("List")` 模式，else 分支传变量 slot（pointer-to-pointer），`push` 对垃圾 data 字段 `realloc` → invalid pointer。
...
- call-site by-owner 索引安装到 consumers。
- ownership action 明细目录安装到 consumers。
- ownership branch-merge 明细目录安装到 consumers。
- Flow transitions-by-flow 明细目录安装到 consumers。
- call-site by-callee 索引安装到 consumers。
- Flow transitions-by-event 明细目录安装到 consumers。
