...
- **Typed storage (de-stringification)**: Not implemented (deferred). **Updated 0.38.26 / ABI v3**: `List<string>` elements are length-bearing `MimiStr` boxes (`{ptr, len}` + `MIMI_STR_MAGIC`, `src/runtime/list_string.rs`). `mimi_str_box` copies the exact input bytes into box-owned storage; the old NUL-terminated `char*` layout and ABI v2 stamp are rejected. General arbitrary-typed payload storage remains deferred.
...
- **Fat pointer (de-global-lock)**: Not implemented (handle registry persists). Map/Set use **generational handles and exclusive internal operation leases** (`src/runtime/handle.rs`): operations on one handle serialize across threads, same-thread reentry fails closed, and destroy defers reclamation until active operations and C lifetime pins drain. The historical `mimi_map_lease_acquire/release` and Set pair only pin lifetime; they do not grant object access. The global handle registry (`Mutex`) still persists — full de-global-lock remains deferred.
...
**Opaque pointer + Generation (thread-safety enforcement)**: Not implemented (deferred). **Updated 0.38.x**: runtime **generation tracking is now implemented** — Flow values carry a `TransitionEpoch` (`src/runtime/epoch.rs`, 0.38.46) packed at Channel/FFI/mailbox escape and checked on the peer side; Map/Set use generational handles (`src/runtime/handle.rs`). The remaining deferred item is the opaque-pointer *ownership* envelope for raw C interop, not generation per se.
