...
- **0.31.25 止血/审查 II**：Verifier。
- **0.31.28 Component 稳定检查点**：ABI fuzz + handle race + 回归。
- **0.31.33 Component 审查**：ABI/Wire/callback/async。
- **0.31.34 SDK conformance 加固**：双 SDK E2E + Wire fuzz。
- **0.31.41 DEBUG**：组合 fuzz、MCDD reference model、性能/内存、flake、standalone binary strip 验证。
- **0.31.42 最终敌对审查**：P0、逃生口、silent fallback、unsupported warning。
- **0.31.43 RC1**：冻结全部 stable profile，只修阻断缺陷。
- **0.31.44 RC2**：第二次干净环境、跨平台、迁移和 SDK E2E。
