...
- **0.31.25**：Component IR / `.mimiabi`，所有 generator 只消费同一 IR。
- **0.31.26**：Native ABI、allocator provenance、typed generational handle 和 concurrent lease。
- **0.31.27**（新增）：Component 稳定检查点 — ABI fuzz + handle race + 回归扫描，0 新 surface。
- **0.31.28**：Subscription quiescence、async cancel exactly-one terminal。
- **0.31.29**：canonical Wire Schema、limits、handshake、revision/conflict、replay。
- **0.31.30**：Rust Safe SDK。
- **0.31.31**：XPU FFI 验证 — extern "C" + #[repr(C)] 调通真实 C 库（OpenVINO/Level Zero/任意 .so），Z3 验证指针非空。
- **0.31.32**：Component 攻击审查，0 新 surface。
- **0.31.33**（新增）：SDK conformance 加固 — Rust SDK E2E + XPU FFI 边界 + Wire fuzz，0 新 surface。

## 0.31.27 稳定检查点详情

> 插入原因：0.31.25–26 从零建立 Component IR 和 Native ABI，生成大量新代码。
...
## 0.31.33 SDK conformance 加固详情

> 插入原因：Rust SDK（0.31.30）和 XPU FFI（0.31.31）各自 E2E 通过后，
