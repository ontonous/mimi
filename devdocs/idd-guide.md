...
> **⚠ 唯一真实锚点（2026-08）**：Mimi 是实验性早期语言，代码库与语法是可替换
> 的实验材料，文档冻结只是 sprint 靶子。**这套 L1/L2/L3 不变量是"能否 break"的
> 唯一裁判**——breaking 的代价不是迁移成本，而是每次 break 后必须重新挣回这套
> 绿。设计思想 + 不变量套件，是 Mimi 仅有的两样长期资产。见 `devdocs/v0.36/README.md`。
