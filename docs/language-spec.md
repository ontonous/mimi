...
| `M::f(...)` 前缀调用 | **不支持**——`::` 保留给 Flow 转移边；工具函数出处由命名自描述约定承担 |
| 内联 `module X { ... }` | **已废止**（0.39.139）：解析期 E0445 定向拒绝；`module` 不再是关键字 |
...
终态（0.39.137 裁决 → 0.39.138 硬拒绝 → 0.39.139 完全退役）：
- 内联 `module` 块**解析期定向拒绝（E0445）**：`module` 已从关键字表退役
  （63→62 词，普通标识符）；item 位形 `module Ident {` 由解析器给出带码
  诊断 + 迁移指引。AST 变体、checker/resolved/cfg/verifier/LSP 的全部下降
  管道、qualified-id 机制均已删除。文件模块是唯一模块形式。
