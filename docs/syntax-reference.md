...
> Mimi 是 MimiSpec 意图描述的生产编译后端。渐进开发特性（$/?/desc/rule）见 MimiSpec 规范。
...
| `std::mymath` | 数学函数：`abs`, `abs_float`, `factorial`, `fibonacci`, `is_prime`, `gcd`, `lcm`, `hypot`, `power`, `sqrt_val`, `floor_val`, `ceil_val`, `round_val`, `collatz_steps`, `mod_pow`, `deg_to_rad`, `rad_to_deg`, `random_int`, `is_power_of_two`, `next_power_of_two` |
...
*Mimi v0.7.0。MimiSpec 语法版本：v0.3.1 / v1.0.0-rc.1。*
