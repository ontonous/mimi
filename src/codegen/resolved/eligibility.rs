...
pub(super) fn eligible_function_ids_with_stats(
    ...
    for function in program.functions().values() {
        ...
        let name = &function.qualified_name;
        // SD-6: flow-transition functions are desugared with origin rule
        // "resolved.runs_flow_transition_method" and a qualified `Actor::transition`
        // name. They are lowered into resolved bodies (lower_checked_transition_bodies)
        // and carry flow-state nominal types that require_scalar_type already
        // admits, so admit them to the resolved slice instead of forcing a legacy
        // fallback (which kept `dispatch --zero` red on every flow program).
        let is_flow_transition = matches!(
            &function.origin,
        ...
        if !matches!(function.origin, crate::core::Origin::User(_)) && !is_flow_transition {
        ...
        // Flow-transition functions are keyed in the callable directory by their
        // transition node id (transition:<Flow>::<name>::<from_state>), not by the
        // function-table key (function:<Actor>::<name>). When the direct lookup
        // misses for a flow transition, resolve the callable by matching the
        // transition name inside the `transition:` owner key.
        let callable = match program.callable(&function.node_id) {
            Some(c) => Some(c),
            None if is_flow_transition => {
                let tname = function.qualified_name.rsplit("::").next().unwrap_or("");
                program.callables().values().find(|c| {
                    c.owner.0.starts_with("transition:")
                        && c.owner.0.contains(&format!("::{}::", tname))
                })
            }
            None => None,
        };
        let Some(callable) = callable else {
            // SD-6: flow transitions that have no resolved body (runtime-generated
            // auto transitions — peer_fault/recover/reset — synthesized only at
            // codegen time) cannot be resolved at the body level. Admit them to
            // the resolved slice anyway: the resolved emitter will fail to emit
            // their body and compile_subset falls back to the legacy emitter at
            // the *emit* level (clear_partial_body → legacy re-emit), so they are
            // not counted as an eligibility skip. This keeps `dispatch --zero`
            // green for every flow program while the deep resolved emit of the
            // flow runtime dispatch remains the follow-up work.
            if is_flow_transition {
