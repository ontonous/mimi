...
impl<'a, 'ctx> NativeMirFunctionEmitter<'a, 'ctx> {
    ...
    pub(super) fn emit_call(
        ...
        if matches!(callee, ResolvedCallee::Extern(_)) {
            return self.emit_ffi_call(result, callee, arguments, subject);
        }
...
mod tests {
    ...
    #[test]
    fn native_ffi_scalar_shape_is_the_single_admitted_physical_family() {
        assert_eq!(
            native_ffi_scalar_shape(MirAbiClass::Integer {
                bits: 32,
                signed: true,
            }),
        ...
        assert_eq!(
            native_ffi_scalar_shape(MirAbiClass::Bool),
