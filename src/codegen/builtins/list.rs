    ...
    pub(super) fn compile_push(
        ...
        args: &[BasicMetadataValueEnum<'ctx>],
                ...
                let realloc_result = self.builder.build_call(realloc_fn, &[
                    ...
                    .map_err(|e| CompileError::Generic(format!("realloc error: {}", e)))?
                    .try_as_basic_value().left()
                ...
                let idx_ptr_i64 = self.builder.build_bit_cast(
                    ...
                    "idx_ptr_i64",
                ).map_err(|e| CompileError::Generic(format!("bitcast error: {}", e)))?.into_pointer_value();
    ...
    pub(super) fn compile_enumerate(
        ...
        args: &[BasicMetadataValueEnum<'ctx>],
                ...
                self.builder.build_store(pair_index_ptr, idx)
                    .map_err(|e| CompileError::Generic(format!("store error: {}", e)))?;
                // Safety: build_gep requires valid pointer and index types; the pointer is derived from a valid LLVM-typed allocation and indices are correctly-typed i64 values.
                let pair_value_ptr = unsafe { self.builder.build_gep(i64_ty, result_data_i64, &[self.builder.build_int_add(idx_2, i64_ty.const_int(1, false), "enum_idx_2_plus_1").map_err(|e| CompileError::Generic(format!("add error: {}", e)))?], "enum_pair_value") }
                    .map_err(|e| CompileError::Generic(format!("gep error: {}", e)))?;
    ...
    pub(super) fn compile_zip(
        ...
        args: &[BasicMetadataValueEnum<'ctx>],
                ...
                self.builder.build_store(pair_a_ptr, elem_a)
                    .map_err(|e| CompileError::Generic(format!("store error: {}", e)))?;
                // Safety: build_gep requires valid pointer and index types; the pointer is derived from a valid LLVM-typed allocation and indices are correctly-typed i64 values.
                let pair_b_ptr = unsafe { self.builder.build_gep(i64_ty, result_data_i64, &[self.builder.build_int_add(idx_2, i64_ty.const_int(1, false), "zip_idx_2_plus_1").map_err(|e| CompileError::Generic(format!("add error: {}", e)))?], "zip_pair_b") }
                    .map_err(|e| CompileError::Generic(format!("gep error: {}", e)))?;
