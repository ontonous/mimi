...
impl<'ctx> CodeGenerator<'ctx> {
    pub(super) fn compile_actor(&mut self, actor: &crate::ast::ActorDef) -> MimiResult<()> {
        ...
        let actor_ty = *self.type_llvm.get(&actor.name)
            .ok_or_else(|| CompileError::Generic(format!("actor type '{}' not found", actor.name)))?;
        ...
        let alloca = match actor_ty {
            ...
            _ => return Err(CompileError::Generic("actor type error".to_string())),
        };
        ...
        self.builder.build_return(Some(&ret_val))
            .map_err(|e| CompileError::Generic(format!("return error: {}", e)))?;
    ...
    pub(super) fn compile_actor_method(&mut self, actor: &crate::ast::ActorDef, method: &FuncDef) -> MimiResult<()> {
        ...
        self.builder.build_store(self_alloca, function.get_nth_param(0)
            .ok_or_else(|| CompileError::Generic("codegen: missing self param in actor method".to_string()))?)
            .map_err(|e| CompileError::Generic(format!("store error: {}", e)))?;
        ...
        for (i, param) in method.params.iter().enumerate() {
            ...
            self.builder.build_store(alloca, function.get_nth_param((i + param_offset) as u32)
                .ok_or_else(|| CompileError::Generic(format!("codegen: missing param {} in actor method", i + param_offset)))?)
                .map_err(|e| CompileError::Generic(format!("store error: {}", e)))?;
            ...
            match stmt {
                ...
                Stmt::If { cond, then_, else_ } => {
                    ...
                    let function = self.current_function()
                        .ok_or_else(|| CompileError::Generic("codegen: no current function for if in actor method".to_string()))?;
                ...
                Stmt::For { var, iterable, body } => {
                    let function = self.current_function()
                        .ok_or_else(|| CompileError::Generic("codegen: no current function for for loop in actor method".to_string()))?;
                    if let Expr::Binary(BinOp::Range, start_expr, end_expr) = iterable {
                        ...
                        return Err(CompileError::Generic("for loop requires range in codegen".to_string()));
                    }
                }
                Stmt::While { cond, body } => {
                    let function = self.current_function()
                        .ok_or_else(|| CompileError::Generic("codegen: no current function for while loop in actor method".to_string()))?;
