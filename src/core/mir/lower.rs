    ...
    if !lowerer.current_is_terminated() && lowerer.errors.is_empty() {
        if let Some(result) = body.root.result.as_deref() {
            ...
            if lowerer.errors.is_empty() {
                ...
                // that ledger mechanically: a single-block body drops the
                // String locals the emitted graph never consumed, so a
                // rebind copy (`let v = s; v`) or an unused side value
                // accounts for its original handle instead of leaking it.
                // R6-1084 widened the face beyond owned-String returns — an
                // `i32`-returning `main` that binds a String it never prints
                // holds the same unaccounted handle.  Consumed-set
                // computation is conservative — a value is only considered
                // free of obligations when no instruction can still consume
                // it.
...
/// one-, two-, three-, four-, five-, six-, or seven-field record parameter, including a non-zero
...
fn materialize_generic_instance(
    ...
    if is_owned_record_projection_drop {
        ...
        .ok_or_else(|| {
            vec![MirLoweringError {
                node_id: subject(),
                message:
                    "owned generic record move/drop projection must have exactly one MIR block"
                        .into(),
            }]
        })?;
        let (project_result, project_base, field) = match block.instructions.as_slice() {
            ...
            [_] => {
                return Err(vec![MirLoweringError {
                    node_id: subject(),
                    message: "owned generic record move/drop projection must contain one direct field Project".into(),
                }])
            }
            _ => {
                return Err(vec![MirLoweringError {
                    node_id: subject(),
                    message: "owned generic record move/drop projection body may contain only one field Project".into(),
                }])
            }
        };
        let base_ty = function
            ...
            .ok_or_else(|| {
                vec![MirLoweringError {
                    node_id: subject(),
                    message: "owned generic record move/drop projection base value is absent"
                        .into(),
                }]
            })?;
        let result_ty = function
            ...
            .ok_or_else(|| {
                vec![MirLoweringError {
                    node_id: subject(),
                    message: "owned generic record move/drop projection result value is absent"
                        .into(),
                }]
            })?;
        let receipt = type_catalog
            ...
            .map_err(|message| {
                vec![MirLoweringError {
                    node_id: subject(),
                    message: format!(
                        "owned generic record move/drop projection contract specialization failed: {message}"
                    ),
                }]
            })?;
        ...
        variant_projection_fallbacks
    {
            ...
            .map(|descriptor| &descriptor.kind)
        {
            ...
            Some(super::types::MirTypeKind::Result)
                if is_owned_result_projection_fallback
        ...
        if is_owned_option_projection_fallback || is_owned_result_projection_fallback {
            let block = function.blocks.get_mut(&block_id).ok_or_else(|| {
                vec![MirLoweringError {
                    node_id: subject(),
                    message: "managed variant fallback entry block is absent during specialization"
                        .into(),
                }]
            })?;
            if block.instructions.len() != 3 || instruction_index != 2 {
                return Err(vec![MirLoweringError {
                    node_id: subject(),
                    message:
                        "managed variant fallback body must contain two Clones and one ProjectOr"
                            .into(),
                }]);
            }
    ...
    let has_record_update = function.blocks.values().any(|block| {
        block
            .instructions
            .iter()
    ...
    let contract = if is_identity {
        ...
        let contract =
            detect_owned_record_projection_drop_contract(&function, type_catalog, &subject())?;
...
pub(crate) fn validate_scalar_variant_projection_fallback_mir(
    ...
    if contract.projection.ownership == super::types::MirOwnership::Move {
        ...
        let valid_move_shape = match receipt.projection.nominal.as_str() {
            "builtin:type:Option" => {
        ...
        if !valid_move_shape {
            return Err(
                "managed generic variant fallback receipt is outside the Move Option/Result contract"
                    .into(),
            );
        }
        return Ok(());
    ...
    let [projection] = projection.as_slice() else {
        ...
        return Err(vec![MirLoweringError {
            ...
            message: if unsupported_arity {
                "generic record projection requires a one-, two-, three-, four-, five-, six-, or seven-field Copy record contract".into()
            } else {
...
fn detect_owned_record_projection_drop_contract(
    function: &MirFunction,
    type_catalog: &MirTypeCatalog,
    subject: &NodeId,
) -> Result<super::types::MirRecordMoveProjectionDropContract, Vec<MirLoweringError>> {
    let [parameter] = function.parameters.as_slice() else {
        return Err(vec![MirLoweringError {
            node_id: subject.clone(),
            message: "owned generic record move/drop projection must have exactly one parameter"
                .into(),
        }]);
    };
    if function.blocks.len() != 1 {
        return Err(vec![MirLoweringError {
            node_id: subject.clone(),
            message: "owned generic record move/drop projection must have exactly one MIR block"
                .into(),
        }]);
    }
    let block = function.blocks.get(&function.entry).ok_or_else(|| {
        vec![MirLoweringError {
            node_id: subject.clone(),
            message: "owned generic record move/drop projection entry block is absent".into(),
        }]
    })?;
    let [MirInstruction {
        kind:
    ...
    if base != parameter {
        return Err(vec![MirLoweringError {
            node_id: subject.clone(),
            message: "owned generic record move/drop projection must consume its record parameter"
                .into(),
        }]);
    }
    let base_ty = function
        ...
        .ok_or_else(|| MirLoweringError {
            node_id: subject.clone(),
            message: "owned generic record move/drop projection base value is absent".into(),
        })
        .map_err(|error| vec![error])?
        .ty
        .clone();
    let result_ty = function
        ...
        .ok_or_else(|| MirLoweringError {
            node_id: subject.clone(),
            message: "owned generic record move/drop projection result value is absent".into(),
        })
        .map_err(|error| vec![error])?
        .ty
        .clone();
    type_catalog
        ...
        .map_err(|message| {
            vec![MirLoweringError {
                node_id: subject.clone(),
                message: format!(
                    "owned generic record move/drop projection receipt is unsupported: {message}"
                ),
            }]
        })?;
    if function.result != result_ty {
        return Err(vec![MirLoweringError {
            node_id: subject.clone(),
            message: "owned generic record move/drop projection requires a direct result identity"
                .into(),
        }]);
    }
    if field != &receipt.projection.field {
        return Err(vec![MirLoweringError {
            node_id: subject.clone(),
            message: "owned generic record move/drop projection field disagrees with receipt"
                .into(),
        }]);
    }
    let MirTerminator::Return {
        ...
        return Err(vec![MirLoweringError {
            node_id: subject.clone(),
            message:
                "owned generic record move/drop projection must directly return its MoveProjectDrop result"
                    .into(),
        }]);
    };
    if returned != result {
        return Err(vec![MirLoweringError {
            node_id: subject.clone(),
            message:
                "owned generic record move/drop projection return value is not the MoveProjectDrop result"
                    .into(),
        }]);
    }
    Ok(receipt.clone())
...
/// Materialize the bounded generic Copy-record update receipt.  The concrete
/// body must contain exactly one `UpdateRecord` with one explicit override and
/// return that value directly; all layout/ABI facts come from the specialized
/// TypeDesc catalog rather than the source record syntax.
fn detect_scalar_record_update_contract(
    function: &MirFunction,
    type_catalog: &MirTypeCatalog,
    subject: &NodeId,
) -> Result<super::types::MirRecordUpdateContract, Vec<MirLoweringError>> {
    let [parameter] = function.parameters.as_slice() else {
        return Err(vec![MirLoweringError {
            node_id: subject.clone(),
            message: "generic record update must have exactly one parameter".into(),
        }]);
    };
    if function.blocks.len() != 1 {
        return Err(vec![MirLoweringError {
            node_id: subject.clone(),
            message: "generic record update must have exactly one MIR block".into(),
        }]);
    }
    let block = function.blocks.get(&function.entry).ok_or_else(|| {
        vec![MirLoweringError {
    ...
    let base_ty = function
        ...
        .ok_or_else(|| MirLoweringError {
            node_id: subject.clone(),
            message: "generic record update base value is absent".into(),
        })
        .map_err(|error| vec![error])?
        .ty
        .clone();
    let result_ty = function
        ...
        .ok_or_else(|| MirLoweringError {
            node_id: subject.clone(),
            message: "generic record update result value is absent".into(),
        })
        .map_err(|error| vec![error])?
        .ty
        .clone();
    let field_types = update_values
        ...
        .map(|value| {
            function
                ...
                .ok_or_else(|| MirLoweringError {
                    node_id: subject.clone(),
                    message: format!("generic record update field '{}' is absent", value.0),
                })
        })
        .collect::<Result<Vec<_>, _>>()
        .map_err(|error| vec![error])?;
    let contract = type_catalog
        ...
        .map_err(|message| {
            vec![MirLoweringError {
                node_id: subject.clone(),
                message: format!("generic record update receipt specialization failed: {message}"),
            }]
        })?;
    if function.result != result_ty {
        return Err(vec![MirLoweringError {
            node_id: subject.clone(),
            message: "generic record update requires a direct result identity".into(),
        }]);
    }
    let MirTerminator::Return {
        value: Some(returned),
        ...
        return Err(vec![MirLoweringError {
            node_id: subject.clone(),
            message: "generic record update must directly return its UpdateRecord result".into(),
        }]);
    };
    if returned != result {
        return Err(vec![MirLoweringError {
...
/// Validate the materialized body behind a `ScalarRecordUpdate` generic
/// instance.  The receipt must agree with the single `UpdateRecord` and its
/// checker-owned TypeDesc/layout facts before any consumer runs.
pub(crate) fn validate_scalar_record_update_mir(
    function: &MirFunction,
    type_catalog: &MirTypeCatalog,
    contract: &super::types::MirRecordUpdateContract,
) -> Result<(), String> {
    let [parameter] = function.parameters.as_slice() else {
        return Err("generic record update must have exactly one parameter".into());
    };
    if function.blocks.len() != 1 {
        return Err("generic record update must have exactly one MIR block".into());
    }
    let block = function
        .blocks
        .get(&function.entry)
    ...
    let base_ty = function
        .values
        .get(base)
        .ok_or_else(|| "generic record update base value is absent".to_string())?
        .ty
        .clone();
    let result_ty = function
        .values
        .get(result)
        .ok_or_else(|| "generic record update result value is absent".to_string())?
        .ty
        .clone();
    let field_types = fields
        ...
        .map(|field| {
            function
                ...
                .get(field)
                .map(|value| value.ty.clone())
                .ok_or_else(|| format!("generic record update field '{}' is absent", field.0))
        })
        .collect::<Result<Vec<_>, _>>()?;
    ...
    if receipt != contract || function.result != result_ty {
        return Err("generic record update receipt disagrees with instance/result identity".into());
    }
    let MirTerminator::Return {
        value: Some(returned),
    } = &block.terminator
    else {
        return Err("generic record update must directly return its UpdateRecord result".into());
    };
    if returned != result {
...
/// Validate the materialized body behind an `OwnedRecordProjectionDrop`
/// generic instance. The selected owned field is returned while every
/// non-Copy sibling is discharged by the receipt attached to
/// `MoveProjectDrop`; no residual ownership is left implicit for consumers.
pub(crate) fn validate_owned_record_projection_drop_mir(
    function: &MirFunction,
    type_catalog: &MirTypeCatalog,
    contract: &super::types::MirRecordMoveProjectionDropContract,
) -> Result<(), String> {
    let [parameter] = function.parameters.as_slice() else {
        return Err(
            "owned generic record move/drop projection must have exactly one parameter".into(),
        );
    };
    if function.blocks.len() != 1 {
        return Err(
            "owned generic record move/drop projection must have exactly one MIR block".into(),
        );
    }
    let block = function.blocks.get(&function.entry).ok_or_else(|| {
        "owned generic record move/drop projection entry block is absent".to_string()
    })?;
    if block.instructions.len() != 1 {
        return Err(
            "owned generic record move/drop projection body may contain only one MoveProjectDrop"
                .into(),
        );
    }
    ...
    if base != parameter {
        return Err(
            "owned generic record move/drop projection must consume its record parameter".into(),
        );
    }
    let base_ty = function
        .values
        .get(base)
        .ok_or_else(|| {
            "owned generic record move/drop projection base value is absent".to_string()
        })?
        .ty
        .clone();
    let result_ty = function
        ...
        .get(result)
        .ok_or_else(|| {
            "owned generic record move/drop projection result value is absent".to_string()
        })?
        .ty
        .clone();
    type_catalog.validate_record_move_projection_drop_receipt(&base_ty, &result_ty, receipt)?;
    if receipt != contract {
        return Err(
            "owned generic record move/drop projection receipt disagrees with instance contract"
                .into(),
        );
    }
    if field != &receipt.projection.field {
        return Err(
            "owned generic record move/drop projection field disagrees with receipt".into(),
        );
    }
    if function.result != result_ty {
        return Err(
            "owned generic record move/drop projection requires a direct result identity".into(),
        );
    }
    let MirTerminator::Return {
        value: Some(returned),
...
impl<'a> Lowerer<'a> {
    ...
    /// R6-1083: close the owned-String ledger with end-of-body drop glue.
    /// The owned-String contracts hold every String parameter and local as a
    /// callee-owned value that must be consumed or dropped before the Return
    /// transfers the result.  Graphs that copy a value (the rebind
    /// `let v = s`, an unused side literal) previously left the original
    /// handle live forever — an unaccounted allocation under the
    /// `mimi_string_free` ABI — so this emits an explicit `Drop` for each
    /// local the emitted graph never consumed, placed before the
    /// terminator.
    ///
    /// The face is deliberately narrow: a type catalog must be present and
    /// the body must still be a single block.  R6-1084 removed the
    /// owned-String return-value condition — an `i32`-returning `main` that
    /// binds a String and never prints it holds the same unaccounted
    /// allocation, so every value-returning single-block body closes its
    /// String ledger (the return value itself only joins the consumed set;
    /// non-String returns contribute nothing to the drop targets).  The
    ...
    fn emit_end_of_body_owned_string_drops(&mut self, return_value: &MirValueId, node_id: &NodeId) {
        ...
        let mut drop_targets: Vec<MirValueId> = self
            ...
            .collect();
        drop_targets.sort();
        // R6-1084: instruction identity is `inst:{role}:{node}` and the glue
        // anchors every Drop at the root result node, so two leftover
        // locals in one body need distinct roles (`drop.0`, `drop.1`, …)
        // the same way explicit `drop(...)` statements already label
        // theirs.
        for (index, target) in drop_targets.into_iter().enumerate() {
            self.emit(
                node_id,
                &format!("drop.{index}"),
    ...
    fn variant_call_abi_contract(
        ...
        let contract = if type_catalog.validate_flat_copy_variant(&result_ty).is_ok() {
            ...
            // `?`. Carry an explicit aggregate-envelope receipt for that
            // helper; the surrounding Flow Try lowering still owns the
            // exact source-return boundary and is not a managed direct-call
            // ABI (`Result<owned, i32>`).
            if self.transition_result.is_some()
                && type_catalog
                    .validate_recoverable_result_variant(&result_ty)
                    .is_ok()
            {
                return match type_catalog.validated_recoverable_result_call_abi_contract(
                    owner,
                    type_arguments,
                    &parameter_types,
                    &result_ty,
...
fn call_builtin_contract(
    ...
    if builtin.as_str() == "session_open" {
