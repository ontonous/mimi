...
/// one-, two-, three-, or four-field record parameter, including a non-zero
...
fn detect_scalar_record_projection_contract(
    ...
    let [(_, _, _)] = projection.as_slice() else {
        let unsupported_arity = function
            ...
            .is_some_and(|arity| !matches!(arity, 1 | 2 | 3 | 4));
        return Err(vec![MirLoweringError {
            node_id: subject.clone(),
            message: if unsupported_arity {
                "generic record projection requires a one-, two-, three-, or four-field Copy record contract".into()
            } else {
    ...
    if !matches!(receipt.arity, 1 | 2 | 3 | 4) || function.result != result_ty {
        return Err(vec![MirLoweringError {
            node_id: subject.clone(),
            message:
                "generic record projection requires one, two, three, or four fields and a direct result identity"
                    .into(),
        }]);
    }
    let MirTerminator::Return {
...
pub(crate) fn validate_scalar_record_projection_mir(
    ...
    if !matches!(contract.arity, 1 | 2 | 3 | 4) || function.result != result_ty {
        return Err(
            "generic record projection requires one, two, three, or four fields and a direct result identity"
                .into(),
        );
