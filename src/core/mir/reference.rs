...
fn validate_instance_table(
    ...
    for (id, instance) in instances {
        ...
        match instance.contract {
            ...
            MirGenericInstanceContract::ScalarVariantProjection { ref contract } => {
                if let Err(message) = super::lower::validate_scalar_variant_projection_mir(
                    function,
                    type_catalog,
                    contract,
                ) {
                    errors.push(super::MirValidationError {
                        subject: id.to_string(),
                        message: format!(
                            "generic MIR Option projection contract is invalid: {message}"
                        ),
                    });
                }
            }
            MirGenericInstanceContract::ScalarVariantProjectionFallback { ref contract } => {
                if let Err(message) = super::lower::validate_scalar_variant_projection_fallback_mir(
                    function,
                    type_catalog,
                    contract,
                ) {
...
mod tests {
    ...
    #[test]
    fn concrete_four_field_heterogeneous_owned_generic_record_projection_tracks_three_residuals() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let program = MirProgram::from_checked_program(&checked)
            .expect("four-field heterogeneous owned residual projection must materialize");
        let instance = program
            .instances()
            .values()
            .next()
        ...
        let MirGenericInstanceContract::OwnedRecordProjectionDrop { contract } = &instance.contract
            ...
            panic!("four-field heterogeneous generic record must carry a residual receipt");
        };
        assert_eq!(contract.projection.arity, 4);
        assert_eq!(contract.projection.name, "value");
        assert_eq!(
            contract
                .residual
    ...
    fn concrete_three_field_generic_record_projection_executes_with_copy_residuals() {
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let program = MirProgram::from_checked_program(&checked)
            .expect("three-field generic record projection must materialize");
        let instance = program
            .instances()
            .values()
            .next()
        ...
        let MirGenericInstanceContract::ScalarRecordProjection { contract } = &instance.contract
            ...
            panic!("three-field generic record projection must carry a record receipt");
        };
        ...
        let target = program
            .functions()
            .get(&instance.function)
            .expect("three-field generic record projection target");
        let parameter_ty = target
            .values
            .get(&target.parameters[0])
            .map(|value| value.ty.clone())
            .expect("three-field record projection parameter TypeDesc");
        let descriptor = program
            .type_catalog()
            .get(&parameter_ty)
            .expect("three-field record projection TypeDesc");
        assert!(matches!(
            descriptor.layout,
            crate::core::mir::types::MirLayout::Record { ref fields, .. }
                if fields.len() == 3
    ...
    #[test]
    fn eight_field_generic_record_projection_fails_closed() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let error = MirProgram::from_checked_program(&checked)
            .expect_err("eight-field generic record projection must remain fail-closed");
        match error {
    ...
    fn concrete_seven_field_generic_record_projection_executes_with_copy_residuals() {
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let program = MirProgram::from_checked_program(&checked)
            .expect("seven-field generic record projection must materialize");
        let instance = program
            .instances()
            .values()
            .next()
        ...
        let MirGenericInstanceContract::ScalarRecordProjection { contract } = &instance.contract
            ...
            panic!("seven-field generic record projection must carry a record receipt");
        };
    ...
    #[test]
    fn concrete_seven_field_repeated_generic_nonzero_projection_preserves_selected_field() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let program = MirProgram::from_checked_program(&checked)
            .expect("repeated-generic nonzero projection must materialize");
        let instance = program
            .instances()
            .values()
            .next()
        ...
        let MirGenericInstanceContract::ScalarRecordProjection { contract } = &instance.contract
            ...
            panic!("repeated-generic nonzero projection must carry a record receipt");
        };
        assert_eq!(contract.name, "mirror");
        assert_eq!(contract.field_index, 2);
        assert_eq!(contract.arity, 7);
        let target = program
            .functions()
            .get(&instance.function)
            .expect("repeated-generic nonzero projection target");
        let parameter_ty = target
            .values
            .get(&target.parameters[0])
            .map(|value| value.ty.clone())
            .expect("repeated-generic nonzero projection parameter TypeDesc");
        let descriptor = program
            .type_catalog()
            .get(&parameter_ty)
            .expect("repeated-generic nonzero projection TypeDesc");
        assert!(matches!(
            descriptor.layout,
            crate::core::mir::types::MirLayout::Record { ref fields, .. }
                if fields.len() == 7
    ...
    #[test]
    fn concrete_five_field_generic_record_projection_executes_with_copy_residuals() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let program = MirProgram::from_checked_program(&checked)
            .expect("five-field generic record projection must materialize");
        let instance = program
            .instances()
            .values()
            .next()
        ...
        let MirGenericInstanceContract::ScalarRecordProjection { contract } = &instance.contract
            ...
            panic!("five-field generic record projection must carry a record receipt");
        };
        assert_eq!(contract.arity, 5);
        assert_eq!(contract.name, "value");
        let target = program
            .functions()
            .get(&instance.function)
            .expect("five-field generic record projection target");
        let parameter_ty = target
            .values
            .get(&target.parameters[0])
            .map(|value| value.ty.clone())
            .expect("five-field record projection parameter TypeDesc");
        let descriptor = program
            .type_catalog()
            .get(&parameter_ty)
