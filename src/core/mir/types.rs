...
pub struct MirVariantProjectionFallbackContract {
    ...
    /// Payload arity of the alternate variant. `Option::None` is zero while
    /// `Result::Err` in the current island is one; consumers must not assume
    /// both runtime payload vectors have the selected arm's arity.
...
impl MirTypeCatalog {
    ...
    /// Materialize a total Copy Result fallback receipt for an admitted scalar
    /// payload. Generic specialization uses this helper after replacing `T`
    /// with a concrete i32/i64/bool TypeDesc; the narrower concrete i32 island
    /// above remains unchanged and keeps its own default-route boundary.
    pub fn validated_copy_result_scalar_projection_fallback_contract(
        &self,
        source_ty: &ResolvedTypeId,
        variant_id: &NodeId,
        field_id: &NodeId,
        ...
        if ok != result_ty || error != result_ty || result_ty != fallback_ty {
            return Err(
                "Result unwrap_or fallback, result and Ok/Err TypeDesc identities must agree"
                    .into(),
            );
        }
        self.validate_copy_scalar(result_ty)?;
        ...
        {
            return Err(
                "Result unwrap_or projection requires the canonical single Copy Ok payload".into(),
            );
        }
        let selected = variants
            .iter()
    ...
    /// therefore the receipt proves the alternate tag without pretending it
    /// has a payload slot or an ownership obligation.
    pub fn validated_copy_option_i32_projection_fallback_contract(
        &self,
        source_ty: &ResolvedTypeId,
        variant_id: &NodeId,
        field_id: &NodeId,
    ...
    pub fn validated_copy_option_scalar_projection_fallback_contract(
        ...
        {
            return Err(
                "Option unwrap_or projection requires the canonical single Copy Some payload"
                    .into(),
            );
        }
        let fallback = variants
            ...
            .ok_or_else(|| "Option unwrap_or requires the canonical None variant".to_string())?;
        let selected = variants
            .iter()
            .find(|variant| variant.id == projection.variant)
            .ok_or_else(|| "Option unwrap_or Some variant is absent from TypeDesc".to_string())?;
        if selected.name != "Some" || selected.discriminant != 1 {
            return Err("Option unwrap_or Some discriminant disagrees with TypeDesc".into());
        }
        Ok(MirVariantProjectionFallbackContract {
            source_ty: source_ty.clone(),
            result_ty: result_ty.clone(),
    ...
    pub fn validate_variant_projection_fallback_receipt(
            ...
            .map(|descriptor| descriptor.kind.clone())
        {
                ...
                if self.get(source_ty).is_some_and(|descriptor| {
                    matches!(
                        ...
                        MirLayout::Result { ok, error, .. }
                            if ok == result_ty
                                && error == result_ty
                                && self.get(ok).is_some_and(|payload| {
                                    payload.kind == MirTypeKind::GenericParameter
                                })
            ...
            {
                self.validated_generic_result_projection_fallback_contract(
                    source_ty,
                    &receipt.projection.variant,
                    &receipt.projection.field,
                    result_ty,
            ...
            Some(kind) => {
                return Err(format!(
                    "variant projection fallback source kind {:?} is outside the canonical Option/Result contract",
                    kind
                ));
            }
            None => return Err("variant projection fallback source TypeDesc is absent".into()),
        };
        if receipt != &expected {
    ...
    /// `List<T>` facade. This is not an admission for execution: the generic
    /// element is opaque until specialization, and the concrete instance
    /// must be revalidated through `validated_list_operation_contract_with_argument`.
    pub(crate) fn validated_generic_list_len_operation_contract(
        &self,
        result_ty: &ResolvedTypeId,
        list_ty: &ResolvedTypeId,
    ...
    /// Materialize the non-executable placeholder receipt used while lowering
    /// a generic `List<T>.concat()` facade. The concrete instance must still
    /// regenerate and validate the full two-input receipt after substituting
    /// `T`.
    pub(crate) fn validated_generic_list_concat_operation_contract(
        &self,
        result_ty: &ResolvedTypeId,
        list_ty: &ResolvedTypeId,
        argument_ty: &ResolvedTypeId,
    ) -> Result<MirListOperationContract, String> {
        let descriptor = self.get(list_ty).ok_or_else(|| {
            format!(
                "generic List operation receiver type '{}' is absent",
        ...
        if result_ty != list_ty || argument_ty != list_ty {
            return Err(
                "generic List.concat placeholder requires one canonical List type for result and both inputs".into(),
            );
        }
        Ok(MirListOperationContract {
            list_ty: list_ty.clone(),
    ...
    /// projection shapes currently admitted: a one-, two-, three-, four-, five-, or six-field `Record<T>`
    ...
    pub(crate) fn validated_generic_record_field_projection_contract(
        ...
        if descriptor.kind != MirTypeKind::Nominal
            ...
            || !matches!(fields.len(), 1 | 2 | 3 | 4 | 5 | 6)
        {
            return Err(
                "generic record projection requires a one-, two-, three-, four-, five-, or six-field Copy record contract"
                    .into(),
            );
        }
        let (field_index, field) = fields
        ...
        let siblings_valid = fields
            .iter()
            .filter(|candidate| candidate.id != field.id)
            .all(|candidate| {
                candidate.ty == *result_ty || self.validate_copy_scalar(&candidate.ty).is_ok()
            });
        ...
        {
            return Err(
                "generic record projection placeholder requires selected field/result GenericParameter identity and Copy-scalar siblings".into(),
            );
        }
        Ok(MirRecordProjectionContract {
            nominal: nominal.clone(),
    ...
    /// Materialize the placeholder receipt used while lowering a generic
    /// `Option<T>`/`Result<T, E>` predicate. The generic payload is
    /// intentionally opaque at this stage, so the concrete flat-Copy payload
    /// check is deferred until instance specialization. The variant family,
    /// identities, arity and predicate are still fixed here; an arbitrary
    /// generic enum cannot use this escape hatch.
    pub(crate) fn validated_generic_variant_predicate_contract(
        &self,
        result_ty: &ResolvedTypeId,
        variant_ty: &ResolvedTypeId,
        predicate: crate::core::mir::MirVariantPredicate,
        ...
        let (variants, expected_kind) = match (&descriptor.layout, expected_nominal) {
            (MirLayout::Option { variants, .. }, "builtin:type:Option") => {
                (variants, MirTypeKind::Option)
            }
            (MirLayout::Result { variants, .. }, "builtin:type:Result") => {
                (variants, MirTypeKind::Result)
            }
            _ => {
                return Err(format!(
                    "generic variant predicate source '{}' is not canonical {}",
                    variant_ty.as_str(),
                    expected_nominal
                        .strip_prefix("builtin:type:")
                        .unwrap_or(expected_nominal)
                ));
            }
        };
        ...
        if variants.len() != 2 {
            return Err(format!(
                "generic variant predicate source '{}' has {} variants; exactly two canonical variants are required",
                variant_ty.as_str(), variants.len()
            ));
        }
        let variant = variants
            .iter()
            ...
            .ok_or_else(|| {
                format!(
                    "generic variant predicate target '{}' is absent from TypeDesc",
                    expected_variant
                )
            })?;
        let alternate = variants
            ...
            .find(|candidate| candidate.id != variant.id)
            .ok_or_else(|| {
                "generic variant predicate alternate variant is absent from TypeDesc".to_string()
            })?;
        let expected_fields = |name: &str| match (expected_nominal, name) {
            ("builtin:type:Option", "Some") => 1,
            ("builtin:type:Option", "None") => 0,
            ("builtin:type:Result", "Ok" | "Err") => 1,
            _ => usize::MAX,
        };
        if variant.fields.len() != expected_fields(variant.name.as_str())
            || alternate.fields.len() != expected_fields(alternate.name.as_str())
        {
            return Err(
                "generic variant predicate variants do not match the canonical family shape".into(),
            );
        }
        let has_generic_payload = variants.iter().any(|candidate| {
            candidate.fields.iter().any(|field| {
                self.get(&field.ty)
        ...
        if !has_generic_payload {
            return Err(
                "generic variant predicate payload must include the canonical GenericParameter"
                    .into(),
            );
        }
        Ok(MirVariantPredicateContract {
    ...
    pub(crate) fn validated_generic_result_projection_fallback_contract(
        ...
        let descriptor = self.get(source_ty).ok_or_else(|| {
            format!(
                "generic Result fallback source type '{}' is absent from MIR TypeDesc catalog",
                source_ty.as_str()
            )
        })?;
        let MirLayout::Result {
            ...
            return Err(format!(
                "generic Result fallback source '{}' has no canonical Result layout",
                source_ty.as_str()
            ));
        };
        if descriptor.kind != MirTypeKind::Result
        ...
        let result = self.get(result_ty).ok_or_else(|| {
            format!(
                "generic Result fallback result type '{}' is absent from MIR TypeDesc catalog",
                result_ty.as_str()
            )
        })?;
        let fallback = self.get(fallback_ty).ok_or_else(|| {
            format!(
                "generic Result fallback operand type '{}' is absent from MIR TypeDesc catalog",
                fallback_ty.as_str()
            )
        })?;
        ...
        if variants.len() != 2 {
            return Err(
                "generic Result fallback source must have exactly canonical Ok/Err variants".into(),
            );
        }
        let selected = variants
            .iter()
            ...
            .ok_or_else(|| {
                "generic Result fallback target must be the canonical Ok variant".to_string()
            })?;
        let alternate = variants
            .iter()
            .find(|variant| {
        ...
        Ok(MirVariantProjectionFallbackContract {
            ...
            discriminant: 0,
            fallback_variant: alternate.id.clone(),
            fallback_variant_name: "Err".into(),
            fallback_discriminant: 1,
            fallback_arity: 1,
        })
    }
