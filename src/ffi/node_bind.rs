...
impl NodeBindGenerator {
    ...
    pub fn generate(&self, extern_funcs: &[ExternFunc]) -> Result<String, std::fmt::Error> {
        ...
        writeln!(out, "// Module: {}", sanitize_comment(&self.module_name))?;
        writeln!(out)?;
    ...
    pub fn generate_dts(&self, extern_funcs: &[ExternFunc]) -> Result<String, std::fmt::Error> {
        ...
        writeln!(
            ...
            sanitize_comment(&self.module_name)
        )?;
        writeln!(out)?;
        ...
        for func in extern_funcs {
            ...
            for (i, p) in func.params.iter().enumerate() {
                ...
                } = &contract.args[i]
                {
                    ...
                    writeln!(
                        out,
                        "typedef struct {{ napi_env env; napi_ref ref; pthread_t thread_id; }} {}_t;",
                        slot
                    )?;
                    writeln!(
                        out,
                        "static __thread {}_t {} = {{NULL, NULL, 0}};",
                        slot, slot
                    )?;
                    let ret_c = self.callback_c_type(ret_type);
                    ...
                    writeln!(
                        out,
                        "    if (!{0}.env || !{0}.ref || {0}.thread_id != pthread_self()) {{",
                        slot
                    )?;
    ...
    fn write_callback_arg_extract(
        ...
        writeln!(out, "    {}.thread_id = pthread_self();", slot)?;
        Ok(())
    }
    ...
    fn write_module_init(
        ...
        writeln!(
            out,
            "NAPI_MODULE({}, Init)",
            sanitize_c_identifier(&self.module_name)
        )?;
        Ok(())
    }
...
/// Convert arbitrary module names into a safe C identifier for N-API.
...
/// Make a value safe to embed in a generated C/TypeScript comment.
