...
impl JniBindGenerator {
    ...
    pub fn generate_c(&self, extern_funcs: &[ExternFunc]) -> Result<String, std::fmt::Error> {
        ...
        writeln!(
            out,
            "// Module: {}",
            sanitize_jni_comment(&self.module_name)
        )?;
        writeln!(out)?;
        writeln!(out, "#include <jni.h>")?;
        writeln!(out, "#include <pthread.h>")?;
    ...
    pub fn generate_java(&self, extern_funcs: &[ExternFunc]) -> Result<String, std::fmt::Error> {
        ...
        writeln!(
            ...
            escape_java_string(&self.module_name)
        )?;
        writeln!(out, "    }}")?;
        writeln!(out)?;
    ...
    fn write_jni_function(
        ...
        writeln!(
            ...
            java_class,
            encode_jni_symbol_component(&func.name)
        )?;
        ...
        for (i, p) in func.params.iter().enumerate() {
            match &contract.args[i] {
                ...
                FfiArgContract::StringTransfer => {
                    ...
                    writeln!(out, "        if ({0}_str == NULL) {{", jname)?;
                    writeln!(
                        out,
                        "            jclass mimi_oom = (*env)->FindClass(env, \"java/lang/OutOfMemoryError\");"
                    )?;
                    writeln!(
                        out,
                        "            if (mimi_oom) (*env)->ThrowNew(env, mimi_oom, \"mimi: out of memory copying string argument\");"
                    )?;
                    writeln!(
                        out,
                    ...
                    writeln!(out, "        }}")?;
                    writeln!(out, "        strcpy({0}_str, {0}_utf);", jname)?;
        ...
        for func in extern_funcs {
            ...
            for (i, p) in func.params.iter().enumerate() {
                ...
                } = &contract.args[i]
                {
                    ...
                    writeln!(out, "    pthread_t thread_id;")?;
                    writeln!(out, "}} {0}_t;", slot)?;
                    writeln!(
                        out,
                        "static __thread {0}_t {0} = {{NULL, NULL, NULL, 0}};",
                        slot
                    )?;
                    ...
                    writeln!(
                        out,
                        "    if (!{0}.env || !{0}.global_ref || !{0}.method || {0}.thread_id != pthread_self()) {{",
                        slot
                    )?;
                    writeln!(
                        out,
                    ...
                    writeln!(out, "    {0}.thread_id = pthread_self();", slot)?;
                    writeln!(out, "}}")?;
                    writeln!(out)?;
                    let cb_teardown = self.callback_teardown_name(&func.name, &p.name);
                    writeln!(out, "static void {}(JNIEnv* env) {{", cb_teardown)?;
                    writeln!(out, "    if ({0}.global_ref) {{ (*env)->DeleteGlobalRef(env, {0}.global_ref); {0}.global_ref = NULL; {0}.env = NULL; {0}.method = NULL; {0}.thread_id = 0; }}", slot)?;
                    writeln!(out, "}}")?;
                    writeln!(out)?;
                }
            }
...
/// JNI exported symbol name encoding (JNI spec Chapter 3, "Resolving Native
/// Method Names"): `_` in the Java method name maps to `_1` in the C symbol,
/// while `_` separating package/class/name is literal.
fn encode_jni_symbol_component(name: &str) -> String {
    name.replace('_', "_1")
}
...
/// Make a value safe to embed in a generated C/Java comment.
fn sanitize_jni_comment(s: &str) -> String {
    s.chars()
        .map(|c| {
            if c == '\n' || c == '\r' || c == '\t' {
                ' '
...
/// Escape a value for a Java string literal (same core escapes as C strings).
