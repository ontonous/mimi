...
pub const E0305: &str = "E0305"; // RETIRED (0.35.39): cannot capture local_shared in parasteps — local_shared removed (four-state shared → two-state). Code preserved (never reused).
...
pub const E0422: &str = "E0422"; // RETIRED (§3-诊断卫生 2026-08-07): flow-state unqualified-name shadowing is advisory-only — migrated to W0402 (warnings must not wear E codes). Code preserved (never reused).
...
pub const W013: &str = "W013"; // newtype used interchangeably with its inner type (transparent-newtype mix)
pub const W014: &str = "W014"; // session free-function API deprecated (use ch.send/recv/close method surface)
...
pub const W0402: &str = "W0402"; // flow state unqualified name shadowed by another flow (qualified name required)
...
pub const MIR_ROUTE_MATERIALIZATION_ERROR_CODE: &str = "MIR-MATERIALIZATION-001";
pub const MIR_ROUTE_COVERAGE_ERROR_CODE: &str = "MIR-COVERAGE-001";
...
const MIR_ROUTE_CODES: [&str; 7] = [
    MIR_ROUTE_RECEIPT_ERROR_CODE,
    MIR_ROUTE_MANIFEST_ERROR_CODE,
    MIR_ROUTE_MATERIALIZATION_ERROR_CODE,
    MIR_ROUTE_COVERAGE_ERROR_CODE,
...
pub fn describe(code: &str) -> &'static str {
    match code {
        ...
        E0305 => "cannot capture local_shared in parasteps (retired: local_shared removed in 0.35.39)",
        ...
        MIR_ROUTE_MATERIALIZATION_ERROR_CODE => "canonical MIR route materialization rejected",
        MIR_ROUTE_COVERAGE_ERROR_CODE => "canonical MIR route coverage rejected",
        ...
        W014 => "session free-function API deprecated (use ch.send/recv/close method surface)",
...
mod tests {
    ...
    fn canonical_mir_route_code_classifier_covers_all_registered_routes() {
        let cases = [
            ...
            super::MIR_ROUTE_COVERAGE_ERROR_CODE,
    ...
    /// 0.31.22-9: 错误码注册表 — 断言无重复码
    /// 错误码一旦分配永不复用，此测试防止意外重复分配。
    #[test]
    fn diagnostic_codes_no_duplicates() {
        ...
        let codes: Vec<&str> = vec![
            ...
            super::E0501,
            super::E0502,
            // Warnings (legacy E06xx)
            super::E0600,
            super::E0601,
            super::E0602,
            // Codegen errors (E07xx)
            super::E0700,
            super::E0702,
            super::E0706,
            super::E0707,
