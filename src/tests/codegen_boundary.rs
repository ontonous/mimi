...
fn cg_deeply_nested_arithmetic() {
    if !can_codegen() { return; }
    let out = compile_and_run("func main() -> i32 { println(1 + (2 + (3 + (4 + (5 + 6))))); 0 }").unwrap();
...
fn cg_string_fstring() {
    if !can_codegen() { return; }
    let out = compile_and_run("func main() -> i32 { let x = 42; println(f\"value={x}\"); 0 }").unwrap();
...
fn cg_is_dir_current() {
    if !can_codegen() { return; }
    let out = compile_and_run("func main() -> i32 { if is_dir(\".\") { println(\"yes\") } else { println(\"no\") } 0 }").unwrap();
...
fn cg_path_join_chain() {
    if !can_codegen() { return; }
    let out = compile_and_run("func main() -> i32 { println(path_join(path_join(\"a\", \"b\"), \"c\")); 0 }").unwrap();
