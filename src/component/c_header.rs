...
fn sanitize_c_identifier(name: &str) -> (String, bool) {
    // C keywords cannot be used as identifiers in generated headers; append
    // an underscore so the output still compiles (batch4-09 P3).
...
pub fn generate_c_header(ir: &ComponentIr) -> String {
    ...
    let (guard_base, _) = sanitize_c_identifier(&ir.identity.name.to_uppercase().replace('-', "_"));
    let guard = format!("{}_ABI_H", guard_base);

    // ── Header preamble ──
    out.push_str(&format!(
        "/* Generated by Mimi Component IR v{} — DO NOT EDIT */\n",
        sanitize_c_comment(&ir.identity.version.to_string())
    ));
    out.push_str(&format!(
        "/* ABI version: {} | Component: {} */\n\n",
        sanitize_c_comment(&ir.identity.abi_version.to_string()),
        sanitize_c_comment(&ir.identity.name)
    ));
...
mod tests {
    ...
    fn c_identifier_validation() {
        ...
        // C keywords are valid identifiers lexically but cannot be used as
        // declared names; sanitizer must append an underscore.
        let (kw, modified) = sanitize_c_identifier("int");
        assert_eq!(kw, "int_");
        assert!(modified);
        let (normal, modified) = sanitize_c_identifier("mimi_list_push");
        assert_eq!(normal, "mimi_list_push");
        assert!(!modified);
    }
