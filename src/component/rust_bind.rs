...
/// Make a string safe to place inside a `//!` or `///` Rust comment:
/// line breaks would otherwise end the comment and inject code.
...
pub fn generate_rust_bindings(ir: &ComponentIr) -> String {
    ...
    out.push_str(&format!(
        ...
        sanitize_rust_comment(&ir.identity.version.to_string())
    ));
    out.push_str(&format!(
        "//! ABI version: {} | Component: {}\n",
        sanitize_rust_comment(&ir.identity.abi_version.to_string()),
        sanitize_rust_comment(&ir.identity.name)
    ));
...
fn emit_rust_type_def(out: &mut String, ty: &AbiTypeDef) {
    match ty {
        ...
        AbiTypeDef::Opaque(o) => {
            ...
            out.push_str(&format!("/// {}\n", sanitize_rust_comment(&o.description)));
