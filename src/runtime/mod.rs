...
/// Recursively serialize a `Set<T>` to a JSON array, sorted by each element's
/// JSON serialization. The VM sorts `Set` elements by their `Display` string;
/// sorting by the JSON serialization matches that order for every scalar,
/// string, float and bool element type (string values are quote-prefixed
/// uniformly, so lexicographic order is preserved).
///
/// `elem_ser_cb` is the per-element recursive serializer generated by the
/// codegen: its `slot` argument is a pointer to the i64 element-handle, exactly
/// the same ABI `mimi_json_join_list` uses for `List` element callbacks.
///
/// # Safety
...
pub unsafe extern "C" fn mimi_json_serialize_set(
    ...
    // Collect handles into an owned Vec so we can hand the callback stable
    // slot pointers (`&i64`) — the same contract as `mimi_json_join_list`.
    let vals: Vec<i64> = set.inner.iter().copied().collect();
    let mut items: Vec<(String, String)> = Vec::with_capacity(vals.len());
    for v in &vals {
        let slot: *const std::ffi::c_void = v as *const i64 as *const std::ffi::c_void;
        let frag = elem_ser_cb(slot);
        let s = if frag.is_null() {
            String::from("null")
        } else {
            let t = cstr_to_string(frag);
            mimi_free(frag as *mut std::ffi::c_void);
            t
        };
        items.push((s.clone(), s));
    }
    // Sort by the JSON serialization to match the VM's `Display`-order sort.
    items.sort_by(|a, b| a.0.cmp(&b.0));
    let mut parts = String::from("[");
    for (i, (json, _)) in items.iter().enumerate() {
        if i > 0 {
            parts.push(',');
        }
        parts.push_str(json);
    ...
    parts.push(']');
    alloc_c_string(&parts)
}

/// Recursively serialize a `Map<K,V>` to a JSON object `{key: value, ...}`,
/// sorted by key. Mimi maps store keys as strings internally, so the JSON
/// object key is the map's string key (rendered unescaped, matching the VM's
/// `to_json` for maps). `val_ser_cb` is the recursive value serializer; its
/// `slot` is a pointer to the i64 value-handle (same ABI as the list callback).
///
/// # Safety
...
pub unsafe extern "C" fn mimi_json_serialize_map(
    ...
    let pairs: Vec<(String, i64)> = map
        ...
        .collect();
    let mut items: Vec<(String, String)> = Vec::with_capacity(pairs.len());
    for (k, vh) in &pairs {
        let slot: *const std::ffi::c_void = vh as *const i64 as *const std::ffi::c_void;
        let frag = val_ser_cb(slot);
        let s = if frag.is_null() {
            String::from("null")
        } else {
            let t = cstr_to_string(frag);
