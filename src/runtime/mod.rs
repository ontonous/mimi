...
struct MimiActorRepr {
    ...
    id: u64,
    /// Mutable actor field storage — a heap-allocated blob that the dispatch
    /// function reads/writes via `self_fields_ptr`.
    fields: Box<[u8]>,
...
thread_local! {
    static CURRENT_ACTOR_ID: std::cell::Cell<u64> = std::cell::Cell::new(0);
}
