...
impl<'a> Interpreter<'a> {
    ...
    pub fn call_named(&mut self, name: &str, args: Vec<Value>) -> Result<Value, InterpError> {
        ...
        match name {
            ...
            "wrapping_add" => self.builtin_wrapping_add(args),
            "wrapping_sub" => self.builtin_wrapping_sub(args),
            "wrapping_mul" => self.builtin_wrapping_mul(args),
            "is_nan" => self.builtin_is_nan(args),
            "is_infinite" => self.builtin_is_infinite(args),
            "is_finite" => self.builtin_is_finite(args),
            "is_close" => self.builtin_is_close(args),
            "f64_eq_exact" => self.builtin_f64_eq_exact(args),
