...
impl<'a> super::Interpreter<'a> {
    ...
    pub(crate) fn builtin_session_pair(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if !args.is_empty() {
    ...
    pub(crate) fn builtin_session_send(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if args.len() != 2 {
            return Err(InterpError::new(
                "session_send expects 2 arguments (channel, value)",
            ));
        }
        let h = match &args[0] {
            Value::Int(n) => *n,
            _ => {
                return Err(InterpError::new(
                    "session_send: channel handle must be integer",
                ))
            }
        };
        let v = match &args[1] {
    ...
    pub(crate) fn builtin_session_recv(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if args.len() != 1 {
            return Err(InterpError::new(
                "session_recv expects 1 argument (channel)",
            ));
        }
        let h = match &args[0] {
            Value::Int(n) => *n,
            _ => {
                return Err(InterpError::new(
                    "session_recv: channel handle must be integer",
                ))
            }
        };
    ...
    pub(crate) fn builtin_session_close(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if args.len() != 1 {
            return Err(InterpError::new(
                "session_close expects 1 argument (channel)",
            ));
        }
        let h = match &args[0] {
            Value::Int(n) => *n,
            _ => {
                return Err(InterpError::new(
                    "session_close: channel handle must be integer",
                ))
            }
        };
    ...
    pub(crate) fn builtin_protocol_methods(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if args.len() != 1 {
            return Err(InterpError::new(
                "protocol_methods expects 1 argument (protocol name)",
            ));
        }
        ...
        let methods: Vec<Value> = self
            ...
            .find_map(|item| match item {
                crate::ast::Item::Protocol(p) if p.name == name => Some(
                    p.transitions
                        .iter()
