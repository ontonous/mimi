...
pub use mir::{
    ...
    compile_mir_program_with_route_receipt,
};
...
"#;
    ...
    #[test]
    fn vm_rejects_forged_new_tuple_register_window() {
        let mut main = FunctionProto::new("main".into(), 0);
        let result = main.alloc_reg();
        main.emit(Op::NewTuple {
            rd: result,
            base: result + 1,
            arity: 1,
        });
        ...
        let error = BytecodeVM::new(std::sync::Arc::new(prog))
            .run()
            .expect_err("a forged tuple source window must fail before source access");
        assert!(
            error
                .to_string()
                .contains("tuple construction register window"),
            "{error}"
        );
    }

    #[test]
    fn vm_rejects_forged_new_tuple_move_register_window() {
        let mut main = FunctionProto::new("main".into(), 0);
        let result = main.alloc_reg();
        main.emit(Op::NewTupleMove {
            rd: result,
            base: result + 1,
            arity: 1,
        });
        ...
        let error = BytecodeVM::new(std::sync::Arc::new(prog))
            .run()
            .expect_err("a forged tuple-move source window must fail before source access");
        assert!(
            error
                .to_string()
                .contains("tuple move construction register window"),
            "{error}"
        );
    }

    #[test]
    fn vm_rejects_forged_new_record_register_window() {
        let mut main = FunctionProto::new("main".into(), 0);
        let result = main.alloc_reg();
        main.emit(Op::NewRecord {
            rd: result,
        ...
        let error = BytecodeVM::new(std::sync::Arc::new(prog))
            .run()
            .expect_err("a forged record source window must fail before source access");
        assert!(
            error
                .to_string()
                .contains("record construction register window"),
            "{error}"
        );
    }

    #[test]
    fn vm_rejects_forged_new_record_move_register_window() {
        let mut main = FunctionProto::new("main".into(), 0);
        let result = main.alloc_reg();
        main.emit(Op::NewRecordMove {
            rd: result,
            type_name: 0,
            base: result + 1,
            count: 1,
        ...
        let error = BytecodeVM::new(std::sync::Arc::new(prog))
            .run()
            .expect_err("a forged record-move source window must fail before source access");
        assert!(
            error
                .to_string()
                .contains("record move construction register window"),
            "{error}"
        );
    }
    ...
    fn vm_rejects_forged_destructure_variant_register_window() {
        ...
        let error = BytecodeVM::new(std::sync::Arc::new(prog)).run().expect_err(
            "a forged variant-destructure target window must fail before source access",
        );
        assert!(
            error
                .to_string()
                .contains("variant destructure register window"),
            "{error}"
        );
    }

    #[test]
    fn vm_rejects_forged_destructure_tuple_register_window() {
        let mut main = FunctionProto::new("main".into(), 0);
        let result = main.alloc_reg();
        ...
        let error = BytecodeVM::new(std::sync::Arc::new(prog))
            .run()
            .expect_err("a forged tuple-destructure target window must fail before source access");
        assert!(
            error
                .to_string()
                .contains("tuple destructure register window"),
            "{error}"
        );
    }
    ...
    #[test]
    fn vm_rejects_forged_call_builtin_argument_window() {
        let source = r#"
        func main() -> i32 {
            println(41)
            0
        ...
        let call = main
            ...
            .expect("println must emit a CallBuiltin instruction");
        *call.0 = register_count;
        *call.1 = 1;

        let error = BytecodeVM::new(program)
            .run()
            .expect_err("a forged builtin-call argument window must fail before fast-path access");
        assert!(
            error
                .to_string()
                .contains("builtin call argument register window"),
            "{error}"
        );
    }

    #[test]
    fn vm_rejects_forged_call_argument_window() {
        let source = r#"
        func add(x: i64) -> i64 {
            x + 1
        }
        ...
        let call = main
            ...
            .expect("direct call must emit a Call instruction");
        *call.0 = register_count;
        *call.1 = 1;

        let error = BytecodeVM::new(program)
            .run()
            .expect_err("a forged call argument window must fail before frame push");
        assert!(
            error.to_string().contains("call argument register window"),
            "{error}"
        );
    }
