...
impl<'a> FunctionEmitter<'a> {
    ...
    fn emit_binary(
        ...
        let opcode = match operand_desc.abi {
            ...
            MirAbiClass::Float { bits: 64 } => match op {
                ...
                // R6-1067: MulFloat/DivFloat already carry the runtime float
                // traps — DivFloat raises the E0801 zero-divisor violation
                // itself, and for mul/div a non-finite operand can never
                // yield a finite result (inf*0=NaN, inf/inf=NaN), so the
    ...
    fn emit_call(
        ...
        let mut call_abi_errors = crate::core::mir::validate_materialized_call_abi(
            callee,
            self.function,
            target,
            result,
    ...
    fn emit_variant_project_or(
        ...
        let contract_idx = self.add_const(ConstValue::VariantProjectionFallback(
            VariantProjectionFallbackShape {
                ...
                field_index,
                arity,
                consuming: receipt.projection.ownership == MirOwnership::Move,
            },
        ));
...
mod tests {
    ...
    #[test]
    fn canonical_mir_rejects_deeper_nested_list_operation_before_backend() {
        let error = run_canonical_differential(include_str!(
    ...
    #[test]
    fn executes_nested_list_concat_with_child_move_through_mir_bytecode() {
        let source = include_str!("../../../tests/fixtures/mir_native_nested_list_concat.mimi");
        let tokens = Lexer::new(source).tokenize().expect("lex");
        let file = Parser::new(tokens).parse_file().expect("parse");
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked).expect("nested List.concat MIR");
        let main = mir
            .functions()
            .get(&crate::core::NodeId("function:main".into()))
            .expect("nested List.concat function");
        let receipt = main
            .blocks
            .values()
            .flat_map(|block| block.instructions.iter())
        ...
        let bytecode = compile_mir_program(&mir).expect("nested List.concat bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("nested List.concat bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(3));
        assert!(matches!(value, Value::Int(3)));
    }
    ...
    #[test]
    fn executes_materialized_four_field_generic_record_projection_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("four-field generic record projection MIR");
        let instance = mir
            .instances()
            .values()
            .next()
        ...
        let bytecode =
            compile_mir_program(&mir).expect("four-field generic record projection bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
    ...
    #[test]
    fn executes_materialized_five_field_generic_record_projection_without_ast() {
        let source = include_str!(
            "../../../tests/fixtures/mir_native_generic_record_projection_five_field.mimi"
        );
        let tokens = Lexer::new(source).tokenize().expect("lex");
        let file = Parser::new(tokens).parse_file().expect("parse");
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("five-field generic record projection MIR");
        let instance = mir
            .instances()
            .values()
            .next()
            .expect("five-field generic record projection instance");
        assert!(matches!(
            instance.contract,
            crate::core::mir::MirGenericInstanceContract::ScalarRecordProjection {
                ref contract
            } if contract.arity == 5 && contract.name == "value"
        ));
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference five-field generic record projection execution");
        let bytecode =
            compile_mir_program(&mir).expect("five-field generic record projection bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("five-field generic record projection bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }

    #[test]
    fn executes_materialized_six_field_generic_record_projection_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("six-field generic record projection MIR");
        let instance = mir
            .instances()
            .values()
            .next()
        ...
        let bytecode =
            compile_mir_program(&mir).expect("six-field generic record projection bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("six-field generic record projection bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }

    #[test]
    fn executes_materialized_seven_field_generic_record_projection_without_ast() {
        let source = include_str!(
        ...
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference seven-field generic record projection execution");
        let bytecode =
            compile_mir_program(&mir).expect("seven-field generic record projection bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("seven-field generic record projection bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }

    #[test]
    fn executes_materialized_seven_field_generic_record_tail_projection_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir =
            MirProgram::from_checked_program(&checked).expect("seven-field tail projection MIR");
        let instance = mir
            .instances()
            .values()
            .next()
        ...
        let bytecode = compile_mir_program(&mir).expect("seven-field tail projection bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("seven-field tail projection bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }

    #[test]
    fn executes_materialized_seven_field_generic_record_projection_with_repeated_generic_fields_without_ast(
    ) {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir =
            MirProgram::from_checked_program(&checked).expect("repeated-generic projection MIR");
        let instance = mir
            .instances()
            .values()
            .next()
        ...
        let bytecode = compile_mir_program(&mir).expect("repeated-generic projection bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("repeated-generic projection bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }
    ...
    #[test]
    fn executes_materialized_owned_generic_record_update_with_copy_residual_without_ast() {
        let source = include_str!(
        ...
        let file = Parser::new(tokens).parse_file().expect("parse");
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked).expect("mixed owned update MIR");
        let instance = mir
            .instances()
            .values()
            .find(|instance| {
        ...
        let bytecode = compile_mir_program(&mir).expect("mixed owned update bytecode");
        assert!(bytecode.ast.is_none());
        assert!(bytecode.functions.iter().any(|function| {
            function
                .code
                .iter()
                .any(|op| matches!(op, Op::UpdateRecordMove { .. }))
        }));
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("mixed owned update bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }

    #[test]
    fn executes_materialized_owned_generic_record_update_with_two_residuals_without_ast() {
        let source = include_str!(
        ...
        let file = Parser::new(tokens).parse_file().expect("parse");
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked).expect("multi-residual update MIR");
        let instance = mir
            .instances()
            .values()
            .find(|instance| {
        ...
        let bytecode = compile_mir_program(&mir).expect("multi-residual update bytecode");
        assert!(bytecode.ast.is_none());
        assert!(bytecode.functions.iter().any(|function| {
            function
                .code
        ...
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("multi-residual update bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }

    #[test]
    fn executes_materialized_owned_generic_record_update_with_three_residuals_without_ast() {
        let source = include_str!(
        ...
        let instance = mir
            ...
            .expect("three-residual owned generic record update instance");
        let crate::core::mir::MirGenericInstanceContract::OwnedRecordUpdate { ref contract } =
            instance.contract
        else {
            unreachable!()
        ...
        assert_eq!(contract.residual.len(), 3);
        assert_eq!(
            contract.residual[0].glue,
            crate::core::mir::types::MirGlueKind::Noop
        );
        assert!(contract.residual[1..]
            .iter()
            .all(|residual| residual.glue == crate::core::mir::types::MirGlueKind::OwnedString));
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference three-residual update execution");
        let bytecode = compile_mir_program(&mir).expect("three-residual update bytecode");
        assert!(bytecode.ast.is_none());
        assert!(bytecode.functions.iter().any(|function| {
            function
                .code
    ...
    fn executes_materialized_generic_option_unwrap_owned_string_without_ast() {
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir =
            MirProgram::from_checked_program(&checked).expect("generic owned Option unwrap MIR");
        let instance = mir
            .instances()
            .values()
            .find(|instance| {
        ...
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference generic owned Option unwrap execution");
        let bytecode = compile_mir_program(&mir).expect("generic owned Option unwrap bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("generic owned Option unwrap bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }
    ...
    #[test]
    fn executes_materialized_generic_result_unwrap_owned_list_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("generic owned Result<List> unwrap MIR");
        let instance = mir
            .instances()
            .values()
            .find(|instance| {
        ...
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference generic owned Result<List> unwrap execution");
        let bytecode =
            compile_mir_program(&mir).expect("generic owned Result<List> unwrap bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("generic owned Result<List> unwrap bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }
    ...
    #[test]
    fn executes_materialized_generic_option_unwrap_or_without_ast() {
        let source =
            include_str!("../../../tests/fixtures/mir_native_generic_option_unwrap_or.mimi");
        let tokens = Lexer::new(source).tokenize().expect("lex");
        let file = Parser::new(tokens).parse_file().expect("parse");
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked).expect("generic Option unwrap_or MIR");
        let instance =
            mir.instances()
                .values()
                .find(|instance| {
        ...
        let bytecode = compile_mir_program(&mir).expect("generic Option unwrap_or bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("bytecode generic Option unwrap_or execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }

    #[test]
    fn generic_option_unwrap_or_none_matches_reference_and_bytecode() {
        let source =
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir =
            MirProgram::from_checked_program(&checked).expect("generic Option unwrap_or None MIR");
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference generic Option unwrap_or None execution");
        let bytecode = compile_mir_program(&mir).expect("generic Option unwrap_or None bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("bytecode generic Option unwrap_or None execution");
        assert_eq!(reference, MirRuntimeValue::Int(7));
        assert!(matches!(value, Value::Int(7)));
    }

    #[test]
    fn executes_materialized_generic_option_unwrap_or_f64_without_ast() {
        let source =
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir =
            MirProgram::from_checked_program(&checked).expect("generic Option<f64> unwrap_or MIR");
        let instance = mir
            .instances()
            .values()
            .find(|instance| {
        ...
        let bytecode = compile_mir_program(&mir).expect("generic Option<f64> unwrap_or bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
    ...
    #[test]
    fn generic_option_unwrap_or_f64_none_matches_reference_and_bytecode() {
        let source = include_str!(
        ...
        let bytecode =
            compile_mir_program(&mir).expect("generic Option<f64> unwrap_or None bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("bytecode generic Option<f64> unwrap_or None execution");
        assert_eq!(reference, MirRuntimeValue::Int(42));
        assert!(matches!(value, Value::Int(42)));
    }

    #[test]
    fn executes_materialized_generic_option_unwrap_or_owned_string_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("generic managed Option unwrap_or MIR");
        let instance = mir
            .instances()
            .values()
            .find(|instance| {
        ...
        let bytecode = compile_mir_program(&mir).expect("managed Option fallback bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("managed Option fallback bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }

    #[test]
    fn executes_materialized_generic_option_unwrap_or_owned_string_none_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("generic managed Option unwrap_or None MIR");
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference managed Option unwrap_or None execution");
        let bytecode = compile_mir_program(&mir).expect("managed Option fallback None bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("managed Option fallback None bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(7));
        assert!(matches!(value, Value::Int(7)));
    }
    ...
    fn executes_materialized_generic_option_unwrap_or_bool_without_ast() {
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir =
            MirProgram::from_checked_program(&checked).expect("generic Option unwrap_or bool MIR");
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference generic Option unwrap_or bool execution");
        let bytecode = compile_mir_program(&mir).expect("generic Option unwrap_or bool bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("bytecode generic Option unwrap_or bool execution");
        assert_eq!(reference, MirRuntimeValue::Int(0));
        assert!(matches!(value, Value::Int(0)));
    }

    #[test]
    fn executes_materialized_generic_option_unwrap_or_i64_without_ast() {
        let source =
            include_str!("../../../tests/fixtures/mir_native_generic_option_unwrap_or_i64.mimi");
        let tokens = Lexer::new(source).tokenize().expect("lex");
        let file = Parser::new(tokens).parse_file().expect("parse");
        let checked = crate::core::check_program(&file).expect("check");
        let mir =
            MirProgram::from_checked_program(&checked).expect("generic Option unwrap_or i64 MIR");
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference generic Option unwrap_or i64 execution");
        let bytecode = compile_mir_program(&mir).expect("generic Option unwrap_or i64 bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("bytecode generic Option unwrap_or i64 execution");
        assert_eq!(reference, MirRuntimeValue::Int(7));
        assert!(matches!(value, Value::Int(7)));
    }
    ...
    fn executes_materialized_generic_result_bool_error_without_ast() {
        ...
        let file = Parser::new(tokens).parse_file().expect("parse");
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked).expect("generic Result<T,bool> MIR");
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference generic Result<bool-error> execution");
        let bytecode = compile_mir_program(&mir).expect("generic Result<T,bool> bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("bytecode generic Result<bool-error> execution");
        assert_eq!(reference, MirRuntimeValue::Int(81));
        assert!(matches!(value, Value::Int(81)));
    }

    #[test]
    fn generic_result_bool_error_unwrap_matches_reference_trap() {
        let source =
        ...
        let bytecode = compile_mir_program(&mir).expect("generic Result<T,bool> trap bytecode");
        assert!(bytecode.ast.is_none());
        let bytecode_error = BytecodeVM::new(bytecode)
            .run_value()
            .expect_err("bytecode Err(true).unwrap must trap");
        assert!(reference_error.to_string().contains("E0800"));
        assert_eq!(bytecode_error.code(), "E0800");
    }
    ...
    fn managed_generic_result_unwrap_or_owned_list_matches_reference_and_bytecode() {
        for (source, expected) in [
            ...
            let checked = crate::core::check_program(&file).expect("check");
            let mir = MirProgram::from_checked_program(&checked)
                .expect("managed Result<List> unwrap_or MIR");
            let reference = MirReferenceInterpreter::new(&mir)
                .execute(&crate::core::NodeId("function:main".into()), &[])
                .expect("reference Result<List> unwrap_or execution");
            let bytecode = compile_mir_program(&mir).expect("Result<List> unwrap_or bytecode");
            assert!(bytecode.ast.is_none());
            let value = BytecodeVM::new(bytecode)
                .run_value()
                .expect("bytecode Result<List> unwrap_or execution");
            assert_eq!(reference, MirRuntimeValue::Int(expected));
            assert!(matches!(value, Value::Int(actual) if actual == expected));
        }
    }
    ...
    fn executes_materialized_two_field_generic_record_projection_without_ast() {
        ...
        let bytecode =
            compile_mir_program(&mir).expect("two-field generic record projection bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("two-field generic record projection bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }
    ...
    #[test]
    fn executes_three_field_generic_record_tail_projection_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir =
            MirProgram::from_checked_program(&checked).expect("three-field tail projection MIR");
        let instance = mir
            .instances()
            .values()
            .next()
        ...
        let bytecode = compile_mir_program(&mir).expect("three-field tail projection bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("three-field tail projection bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }
    ...
    #[test]
    fn executes_owned_generic_record_list_projection_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("owned generic Record<List<i32>> projection MIR");
        let instance = mir
            .instances()
            .values()
            .find(|instance| {
        ...
        let bytecode = compile_mir_program(&mir).expect("owned generic Record<List<i32>> bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("bytecode owned generic Record<List<i32>> projection execution");
        assert_eq!(reference, MirRuntimeValue::Int(42));
        assert!(matches!(value, Value::Int(42)));
    }

    #[test]
    fn executes_owned_generic_record_list_projection_with_string_residual_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("owned generic Record<List<i32>> residual MIR");
        let instance = mir
            .instances()
            .values()
            .find(|instance| {
            ...
            .expect("owned generic residual instance");
        let target = mir
            .functions()
            .get(&instance.function)
            .expect("owned generic residual target");
        assert!(target.canonical_text().contains("move_project_drop"));
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference owned generic residual execution");
        let bytecode = compile_mir_program(&mir).expect("owned generic residual bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("bytecode owned generic residual execution");
        assert_eq!(reference, MirRuntimeValue::Int(42));
        assert!(matches!(value, Value::Int(42)));
    }

    #[test]
    fn executes_owned_generic_record_list_projection_with_list_residual_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("owned generic Record<List<i32>> List residual MIR");
        let instance = mir
            .instances()
            .values()
            .find(|instance| {
            ...
            .expect("owned generic List residual instance");
        let target = mir
            .functions()
            .get(&instance.function)
            .expect("owned generic List residual target");
        assert!(target.canonical_text().contains("move_project_drop"));
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference owned generic List residual execution");
        let bytecode = compile_mir_program(&mir).expect("owned generic List residual bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("owned generic List residual bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(42));
        assert!(matches!(value, Value::Int(42)));
    }

    #[test]
    fn executes_owned_generic_record_list_projection_with_two_list_residual_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("owned generic Record<List<i32>> two-List residual MIR");
        let instance = mir
            .instances()
            .values()
            .find(|instance| {
            ...
            .expect("owned generic two-List residual instance");
        let target = mir
            .functions()
            .get(&instance.function)
            .expect("owned generic two-List residual target");
        assert!(target.canonical_text().contains("move_project_drop"));
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference owned generic two-List residual execution");
        let bytecode = compile_mir_program(&mir).expect("owned generic two-List residual bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("owned generic two-List residual bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(42));
        assert!(matches!(value, Value::Int(42)));
    }

    #[test]
    fn executes_owned_generic_record_list_string_residual_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("owned generic Record<List<i32>> List/String residual MIR");
        let instance = mir
            .instances()
            .values()
            .find(|instance| {
        ...
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference owned generic List/String residual execution");
        let bytecode =
            compile_mir_program(&mir).expect("owned generic List/String residual bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("owned generic List/String residual bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(42));
        assert!(matches!(value, Value::Int(42)));
    }

    #[test]
    fn executes_owned_generic_record_list_two_string_residual_without_ast() {
        let source = include_str!(
            "../../../tests/fixtures/mir_native_generic_record_owned_list_two_string_residual.mimi"
        );
        let tokens = Lexer::new(source).tokenize().expect("lex");
        let file = Parser::new(tokens).parse_file().expect("parse");
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("owned generic Record<List<i32>> List/two-String residual MIR");
        let instance = mir
            .instances()
            .values()
            .find(|instance| {
            ...
            .expect("owned generic List/two-String residual instance");
        let target = mir
            .functions()
            .get(&instance.function)
            .expect("owned generic List/two-String residual target");
        assert!(target.canonical_text().contains("move_project_drop"));
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference owned generic List/two-String residual execution");
        let bytecode =
            compile_mir_program(&mir).expect("owned generic List/two-String residual bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("owned generic List/two-String residual bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(42));
        assert!(matches!(value, Value::Int(42)));
    }
    ...
    #[test]
    fn executes_owned_generic_record_set_residual_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("owned generic Record<Set<i32>> Set residual MIR");
        let instance = mir
            .instances()
            .values()
            .find(|instance| {
        ...
        let bytecode = compile_mir_program(&mir).expect("owned generic Set residual bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("owned generic Set residual bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(42));
        assert!(matches!(value, Value::Int(42)));
    }

    #[test]
    fn executes_owned_generic_record_set_scalar_family_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("owned generic Record<Set<i64|bool>> Set residual MIR");
        let instances = mir
            .instances()
            .values()
            .filter(|instance| {
        ...
        for instance in instances {
            let target = mir
                .functions()
                .get(&instance.function)
                .expect("owned generic Set scalar-family target");
            assert!(target.canonical_text().contains("move_project_drop"));
        }
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference owned generic Set scalar-family execution");
        let bytecode = compile_mir_program(&mir).expect("owned generic Set scalar-family bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("owned generic Set scalar-family bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(42));
        assert!(matches!(value, Value::Int(42)));
    }
    ...
    #[test]
    fn executes_owned_generic_record_projection_with_residual_drop_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("owned generic record residual projection MIR");
        let instance = mir
            .instances()
            .values()
            .next()
        ...
        let bytecode = compile_mir_program(&mir).expect("owned generic record residual bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("bytecode owned generic record residual execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }
    ...
    #[test]
    fn executes_heterogeneous_owned_generic_record_projection_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("heterogeneous owned generic residual projection MIR");
        let instance = mir
            .instances()
            .values()
            .next()
        ...
        let target = mir
            .functions()
            .get(&instance.function)
            .expect("heterogeneous owned generic record target");
        assert!(target.canonical_text().contains("move_project_drop"));
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference heterogeneous owned residual execution");
        let bytecode = compile_mir_program(&mir).expect("heterogeneous owned residual bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("bytecode heterogeneous owned residual execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }

    #[test]
    fn executes_three_field_heterogeneous_owned_generic_record_projection_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("three-field heterogeneous owned projection MIR");
        let instance = mir
            .instances()
            .values()
            .next()
            .expect("three-field heterogeneous owned projection instance");
        assert!(matches!(
            instance.contract,
            crate::core::mir::MirGenericInstanceContract::OwnedRecordProjectionDrop {
                ref contract
        ...
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference three-field heterogeneous owned projection execution");
        let bytecode =
            compile_mir_program(&mir).expect("three-field heterogeneous owned projection bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("bytecode three-field heterogeneous owned projection execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }

    #[test]
    fn executes_four_field_heterogeneous_owned_generic_record_projection_without_ast() {
        let source = include_str!(
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir = MirProgram::from_checked_program(&checked)
            .expect("four-field heterogeneous owned projection MIR");
        let instance = mir
            .instances()
            .values()
            .next()
            .expect("four-field heterogeneous owned projection instance");
        assert!(matches!(
            instance.contract,
            crate::core::mir::MirGenericInstanceContract::OwnedRecordProjectionDrop {
                ref contract
        ...
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference four-field heterogeneous owned projection execution");
        let bytecode =
            compile_mir_program(&mir).expect("four-field heterogeneous owned projection bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("bytecode four-field heterogeneous owned projection execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }
    ...
    fn executes_owned_generic_record_projection_rvalue_without_ast() {
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let mir =
            MirProgram::from_checked_program(&checked).expect("owned generic record rvalue MIR");
        let reference = MirReferenceInterpreter::new(&mir)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference owned generic record rvalue execution");
        let bytecode = compile_mir_program(&mir).expect("owned generic record rvalue bytecode");
        assert!(bytecode.ast.is_none());
        let value = BytecodeVM::new(bytecode)
            .run_value()
            .expect("owned generic record rvalue bytecode execution");
        assert_eq!(reference, MirRuntimeValue::Int(41));
        assert!(matches!(value, Value::Int(41)));
    }
...
"#;
    ...
    #[test]
    fn canonical_scalar_ffi_bytecode_rejects_duplicate_compiler_binding_site() {
        let source = include_str!("../../../tests/fixtures/mir_scalar_ffi_labs.mimi");
        let file = Parser::new(Lexer::new(source).tokenize().expect("lex scalar FFI"))
            .parse_file()
            .expect("parse scalar FFI");
        ...
        forged.canonical_ffi_bindings.push(binding);
        let error = BytecodeVM::new(bytecode)
            .run_value()
            .expect_err("duplicate compiler binding site must fail before loading");
        assert!(
            error.to_string().contains("duplicate call-site binding"),
            "{error}"
        );
    }

    #[test]
    fn canonical_scalar_ffi_bytecode_rejects_binding_site_outside_emitted_functions() {
        let source = include_str!("../../../tests/fixtures/mir_scalar_ffi_labs.mimi");
        let file = Parser::new(Lexer::new(source).tokenize().expect("lex scalar FFI"))
            .parse_file()
            .expect("parse scalar FFI");
        ...
        let mir = MirProgram::from_checked_program(&checked).expect("canonical scalar FFI MIR");
        let mut bytecode = compile_mir_program(&mir).expect("canonical scalar FFI bytecode");
        std::sync::Arc::make_mut(&mut bytecode).canonical_ffi_bindings[0].function = u32::MAX;
        let error = BytecodeVM::new(bytecode)
            .run_value()
            .expect_err("binding outside emitted functions must fail before loading");
        assert!(
            error
                .to_string()
    ...
    #[test]
    fn canonical_scalar_ffi_bytecode_rejects_binding_site_on_non_ffi_instruction() {
        let source = include_str!("../../../tests/fixtures/mir_scalar_ffi_labs.mimi");
        let file = Parser::new(Lexer::new(source).tokenize().expect("lex scalar FFI"))
            .parse_file()
            .expect("parse scalar FFI");
        ...
        let non_ffi_pc = forged.functions[function_idx]
            .code
            .iter()
            .position(|op| !matches!(op, Op::CallCanonicalExtern { .. }))
            .expect("fixture must contain a non-FFI instruction");
        forged.canonical_ffi_bindings[0].pc = non_ffi_pc as u32;
        let error = BytecodeVM::new(bytecode)
            .run_value()
            .expect_err("binding on a non-FFI instruction must fail before loading");
        assert!(
            error
                .to_string()
                .contains("does not point to CallCanonicalExtern"),
            "{error}"
        );
    }
    ...
    #[test]
    fn canonical_scalar_ffi_bytecode_rejects_forged_function_register_count() {
        let source = include_str!("../../../tests/fixtures/mir_scalar_ffi_labs.mimi");
        let file = Parser::new(Lexer::new(source).tokenize().expect("lex scalar FFI"))
            .parse_file()
            .expect("parse scalar FFI");
        ...
        let function_idx = binding.function as usize;
        let forged = std::sync::Arc::make_mut(&mut bytecode);
        forged.functions[function_idx].register_count = binding.register_count.saturating_sub(1);
        let error = BytecodeVM::new(bytecode)
            .run_value()
