...
impl BytecodeVM {
    ...
    fn validate_canonical_ffi_program(&self) -> Result<(), InterpError> {
        ...
        for (function_idx, proto) in self.program.functions.iter().enumerate() {
            for (pc, op) in proto.code.iter().enumerate() {
                ...
                let expected_instruction = match proto.constants.get(*instruction as usize) {
                    ...
                    Some(_) => {
                        return Err(InterpError::new(format!(
                            "canonical FFI instruction identity constant {instruction} at function '{}' pc {pc} is not a string",
                            proto.name
                        )))
                    }
                    None => {
                        return Err(InterpError::new(format!(
                            "canonical FFI instruction identity constant {instruction} out of range at function '{}' pc {pc}",
                            proto.name
                        )))
                    }
                };
                ...
                if descriptor.instruction != *expected_instruction {
                    return Err(InterpError::new(format!(
                        "canonical FFI descriptor index {extern_idx} instruction '{}' disagrees with bytecode instruction '{}' at function '{}' pc {pc}",
                        descriptor.instruction, expected_instruction, proto.name
                    )));
                }
                if let Some((previous_function, previous_pc)) = references[*extern_idx as usize] {
                    return Err(InterpError::new(format!(
                        "canonical FFI descriptor index {extern_idx} is referenced by multiple bytecode call sites (function #{} pc {previous_pc} and function #{} pc {pc})",
                        previous_function, function_idx
                    )));
                }
                references[*extern_idx as usize] = Some((function_idx, pc));
            }
        }
        ...
        if call_count == 0 {
            if !descriptors.is_empty() {
                return Err(InterpError::new(format!(
                    "canonical FFI descriptor table has {} unreferenced entries",
                    descriptors.len()
                )));
            }
            return Ok(());
        ...
        for (index, reference) in references.iter().enumerate() {
            if reference.is_none() {
                return Err(InterpError::new(format!(
                    "canonical FFI descriptor index {index} is unreferenced by bytecode"
                )));
            }
        }
        Ok(())
            ...
            match op {
                ...
                Op::Call {
                    ...
                    if args_end.map_or(true, |end| end > register_count) {
                        return Err(InterpError::new(format!(
                            "call argument register window base {} count {} exceeds frame with {} register(s)",
                ...
                Op::NewRecord {
                    ...
                    if source_end.map_or(true, |end| end > register_count) {
                        return Err(InterpError::new(format!(
                            "record construction register window base {} count {} exceeds frame with {} register(s)",
                ...
                Op::NewRecordMove {
                    ...
                    if source_end.map_or(true, |end| end > register_count) {
                        return Err(InterpError::new(format!(
                            "record move construction register window base {} count {} exceeds frame with {} register(s)",
                            base, count, register_count
                        )));
                    }
                    let type_name_str = match &proto.constants.get(type_name as usize) {
                ...
                Op::DestructureVariantMove {
                    ...
                    let register_count = self.cur_frame().regs.len();
                    let target_end = (base as usize).checked_add(arity as usize);
                    if target_end.map_or(true, |end| end > register_count) {
                        return Err(InterpError::new(format!(
                            "variant destructure register window base {} count {} exceeds frame with {} register(s)",
                            base, arity, register_count
                        )));
                    }
                    ...
                    let expected_tag = match proto.constants.get(variant_tag as usize) {
                        ...
                        None => {
                            return Err(InterpError::new(format!(
                                "variant destructure: tag constant {} is absent",
                                variant_tag
                            )));
                        }
                    };
                ...
                Op::VariantMoveGet {
                    ...
                    let (actual_tag, payload_len, identity) = match self.get_reg(ra) {
                        ...
                        value => {
                            return Err(InterpError::new(format!(
                                "variant move get: expected Variant, got {}",
                                value
                            )))
                        }
                    };
                    let expected_tag = match proto.constants.get(variant_tag as usize) {
                        ...
                        Some(_) => {
                            return Err(InterpError::new(format!(
                                "variant move get: tag constant {} is not a string",
                                variant_tag
                            )))
                        }
                        None => {
                            return Err(InterpError::new(format!(
                                "variant move get: tag constant {} is absent",
                                variant_tag
                            )))
                        }
                    };
                    if actual_tag != expected_tag {
                        return Err(InterpError::new(format!(
                            "E0800: canonical MIR consuming direct variant projection expected active variant '{}', got '{}'",
                            expected_tag, actual_tag
                        )));
                    }
                    let (expected_nominal, expected_variant, expected_arity) = match proto
                        .constants
                        .get(shapes as usize)
                    {
                        Some(ConstValue::VariantShapes(shapes)) => {
                            let Some(shape) = shapes.iter().find(|shape| shape.tag == actual_tag)
                            else {
                                return Err(InterpError::new(format!(
                                        "variant move get: tag '{}' is absent from canonical shape table",
                                        actual_tag
                                    )));
                            };
                            (shape.nominal.clone(), shape.variant.clone(), shape.arity)
                        ...
                        Some(_) => {
                            return Err(InterpError::new(format!(
                                "variant move get: shapes constant {} is not a VariantShapes table",
                                shapes
                            )))
                        }
                        None => {
                            return Err(InterpError::new(format!(
                                "variant move get: shapes constant {} is absent",
                                shapes
                            )))
                        }
                    };
                    if let Some((nominal, variant)) = identity {
                        if nominal != expected_nominal || variant != expected_variant {
                            return Err(InterpError::new(format!(
                                "variant move get: canonical identity for tag '{}' disagrees with shape table",
                                actual_tag
                            )));
                        }
                    }
                    if payload_len != expected_arity as usize {
                        return Err(InterpError::new(format!(
                            "variant move get: payload arity {} disagrees with canonical arity {}",
                            payload_len, expected_arity
                        )));
                    }
                    if (idx as usize) >= expected_arity as usize {
                        return Err(InterpError::index_out_of_bounds(format!(
                            "variant move field index {} out of bounds (canonical arity {})",
                            idx, expected_arity
                        )));
                    }
                    let source = std::mem::replace(self.get_reg_mut(ra), Value::Unit);
                    let elem = match source {
                        Value::Variant(_, mut fields) => {
                            std::mem::replace(&mut fields[idx as usize], Value::Unit)
                        }
                        Value::CanonicalVariant { mut payload, .. } => {
                            std::mem::replace(&mut payload[idx as usize], Value::Unit)
                        }
                        _ => unreachable!("variant move get source changed during validation"),
                    };
                    self.set_reg(rd, elem);
                }
