...
fn eval_materialized_call(
    ...
    match &instance.contract {
        ...
        crate::core::mir::MirGenericInstanceContract::ScalarRecordUpdate { contract } => {
            eval_materialized_record_update_call(
                function,
                program,
                catalog,
                state,
...
fn eval_materialized_variant_projection_fallback_call(
    ...
    if contract.projection.ownership == MirOwnership::Move {
        ...
        catalog.validate_generic_variant_projection_fallback_arguments(type_arguments, contract)?;
    }
    let target = program.functions().get(target_owner).ok_or_else(|| {
        format!(
...
/// Symbolically consume a materialized generic flat Copy-record update.  The
/// target body and update receipt are checked first; the concrete override
/// value is represented symbolically, while the caller's record identity and
/// layout remain checker-owned facts throughout.
fn eval_materialized_record_update_call(
    function: &MirFunction,
    program: &MirProgram,
    catalog: &crate::core::mir::types::MirTypeCatalog,
    state: &mut SymbolicState,
    ...
    type_arguments: &[crate::core::ResolvedTypeId],
    arguments: &[MirValueId],
    contract: &crate::core::mir::types::MirRecordUpdateContract,
) -> Result<(), String> {
    let target = program.functions().get(target_owner).ok_or_else(|| {
        format!(
            "MIR verifier record update target '{}' is absent",
            target_owner.0
        )
    })?;
    crate::core::mir::lower::validate_scalar_record_update_mir(target, catalog, contract)?;
    catalog.validate_scalar_generic_arguments(type_arguments)?;
    if arguments.len() != 1 || target.parameters.len() != 1 {
        return Err("MIR verifier record update call requires one argument".into());
    }
    let result = result
        .as_ref()
        .ok_or_else(|| "MIR verifier record update call must produce a result".to_string())?;
    if function
        .values
        .get(result)
        .is_none_or(|value| value.ty != target.result)
    ...
    let argument_info = function.values.get(argument).ok_or_else(|| {
        format!(
            "MIR verifier record update argument '{}' is absent",
            argument
        )
    })?;
    let parameter = &target.parameters[0];
    let parameter_info = target
        .values
        .get(parameter)
        .ok_or_else(|| "MIR verifier record update parameter TypeDesc is absent".to_string())?;
    if argument_info.ty != parameter_info.ty || argument_info.ty != contract.source_ty {
        return Err("MIR verifier record update argument disagrees with TypeDesc".into());
    }
    let base = state.values.get(argument).cloned().ok_or_else(|| {
        format!(
            "MIR verifier record update argument '{}' is not defined",
            argument
        )
    })?;
    if !symbolic_matches_type(catalog, &argument_info.ty, &base) {
        return Err("MIR verifier record update argument has the wrong symbolic shape".into());
    }
    let MirInstructionKind::UpdateRecord {
        ...
        .ok_or_else(|| "MIR verifier record update target has no UpdateRecord".to_string())?
        .kind
        .clone()
    else {
        return Err("MIR verifier record update target has a non-record UpdateRecord".into());
    };
    let symbolic_updates = update_values
        ...
        .map(|value| {
            let ty = target
                .values
                .get(value)
                .ok_or_else(|| format!("MIR verifier record update value '{}' is absent", value))?
                .ty
                .clone();
...
/// Symbolically consume a materialized generic record projection whose owned
/// field is returned and whose remaining owned siblings are discharged by the
/// explicit `MoveProjectDrop` receipt.  The target body is explored instead of
/// reconstructing a partial-move policy in the verifier, so the same residual
/// TypeDesc/glue contract is shared with reference, bytecode, and native MIR
/// consumers.
fn eval_materialized_owned_record_projection_drop_call(
    function: &MirFunction,
    program: &MirProgram,
    catalog: &crate::core::mir::types::MirTypeCatalog,
    state: &mut SymbolicState,
    ...
    type_arguments: &[crate::core::ResolvedTypeId],
    arguments: &[MirValueId],
    contract: &crate::core::mir::types::MirRecordMoveProjectionDropContract,
) -> Result<(), String> {
    let target = program.functions().get(target_owner).ok_or_else(|| {
        format!(
            "MIR verifier owned record move/drop projection target '{}' is absent",
            target_owner.0
        )
    })?;
    ...
    if arguments.len() != 1 || target.parameters.len() != 1 {
        return Err(
            "MIR verifier owned record move/drop projection call requires one argument".into(),
        );
    }
    let result = result.as_ref().ok_or_else(|| {
        "MIR verifier owned record move/drop projection call must produce a result".to_string()
    })?;
    if function
        .values
        .get(result)
    ...
    if target.result != contract.result_ty || target.result != contract.projection.field_ty {
        return Err(
            "MIR verifier owned record move/drop projection result disagrees with receipt".into(),
        );
    }
    let argument = &arguments[0];
    let argument_info = function.values.get(argument).ok_or_else(|| {
        format!(
            "MIR verifier owned record move/drop projection argument '{}' is absent",
            argument
        )
    })?;
    let parameter = &target.parameters[0];
    let parameter_info = target.values.get(parameter).ok_or_else(|| {
        "MIR verifier owned record move/drop projection parameter TypeDesc is absent".to_string()
    })?;
    if argument_info.ty != parameter_info.ty {
        return Err(
            "MIR verifier owned record move/drop projection argument disagrees with TypeDesc"
                .into(),
        );
    }
    let value = state.values.remove(argument).ok_or_else(|| {
        format!(
            "MIR verifier owned record move/drop projection argument '{}' is not defined",
            argument
        )
    })?;
    if !symbolic_matches_type(catalog, &argument_info.ty, &value) {
        return Err(
            "MIR verifier owned record move/drop projection argument has the wrong symbolic shape"
                .into(),
        );
    }
    let caller_constraints = state.constraints.clone();
    ...
    if !traps.is_empty() {
        return Err(
            "MIR verifier owned record move/drop projection call has a trapping execution path"
                .into(),
        );
    }
    let [returned] = returns.as_slice() else {
        return Err(
            "MIR verifier owned record move/drop projection call must have exactly one return path"
                .into(),
        );
    };
    state.constraints = returned.constraints.clone();
...
mod tests {
    ...
    #[test]
    fn verifier_proves_move_owned_result_i64_bool_list_calls_from_canonical_mir() {
        let source = include_str!("../../tests/fixtures/mir_result_list_i64_bool_call_return.mimi");
        let tokens = Lexer::new(source).tokenize().expect("lex");
        let file = Parser::new(tokens).parse_file().expect("parse");
        let checked = crate::core::check_program(&file).expect("check");
        let program = MirProgram::from_checked_program(&checked)
            .expect("move-owned Result<List<i64|bool>, i32> calls must be canonical MIR");
        let results = verify_program(&program, "result-list-i64-bool-call-source-hash".into())
            .expect("move-owned Result<List<i64|bool>, i32> verifier result");
        for owner in ["function:use_i64", "function:use_bool"] {
            let result = results
                .iter()
                .find(|result| result.func_name == owner)
                .expect("direct managed Result verification result");
            assert_eq!(
                result.status,
                crate::verifier::VerifStatus::Proven,
                "{owner}: {}",
        ...
        let value = MirReferenceInterpreter::new(&program)
            .execute(&crate::core::NodeId("function:main".into()), &[])
            .expect("reference move-owned Result<List<i64|bool>, i32> execution");
        assert_eq!(value, MirRuntimeValue::Int(48));
    }
    ...
    #[test]
    fn verifier_proves_non_copy_option_string_unwrap_projection() {
        let source = include_str!("../../tests/fixtures/mir_native_option_string_unwrap.mimi");
        let tokens = Lexer::new(source).tokenize().expect("lex");
        let file = Parser::new(tokens).parse_file().expect("parse");
        let checked = crate::core::check_program(&file).expect("check");
        let program = MirProgram::from_checked_program(&checked)
            .expect("canonical Option<string>.unwrap MIR");
        let function = program
            .functions()
            .get(&crate::core::NodeId("function:unwrap_owned".into()))
            .expect("unwrap_owned MIR");
        assert!(function.blocks.values().any(|block| {
            block.instructions.iter().any(|instruction| {
                matches!(
                    instruction.kind,
                    crate::core::mir::MirInstructionKind::VariantProjectMove {
                        contract: Some(_),
                        ..
                    }
                )
        ...
        let results = verify_program(&program, "option-string-unwrap-source-hash".into())
            .expect("verifier should prove the consuming projection");
        let result = results
            .iter()
            .find(|result| result.func_name.ends_with("unwrap_owned"))
            .expect("unwrap_owned verification result");
        assert_eq!(result.status, crate::verifier::VerifStatus::Proven);
    }
    ...
    #[test]
    fn verifier_proves_result_i32_i32_unwrap_copy_projection() {
        let source = include_str!("../../tests/fixtures/mir_native_result_i32_unwrap.mimi");
        let tokens = Lexer::new(source).tokenize().expect("lex");
        let file = Parser::new(tokens).parse_file().expect("parse");
        let checked = crate::core::check_program(&file).expect("check");
        let program = MirProgram::from_checked_program(&checked)
            .expect("canonical Result<i32, i32>.unwrap MIR");
        let function = program
            .functions()
            .get(&crate::core::NodeId("function:unwrap_copy".into()))
            .expect("unwrap_copy MIR");
        ...
        let results = verify_program(&program, "result-i32-i32-unwrap-source-hash".into())
            .expect("verifier should prove the Copy Result projection");
        let result = results
            .iter()
            .find(|result| result.func_name.ends_with("unwrap_copy"))
            .expect("unwrap_copy verification result");
    ...
    fn verifier_proves_option_bool_unwrap_copy_projection() {
        ...
        let checked = crate::core::check_program(&file).expect("check");
        let program =
            MirProgram::from_checked_program(&checked).expect("canonical Option<bool>.unwrap MIR");
        let function = program
            .functions()
            .get(&crate::core::NodeId("function:unwrap_copy".into()))
            .expect("unwrap_copy MIR");
        ...
        let results = verify_program(&program, "option-bool-unwrap-source-hash".into())
            .expect("verifier should prove the bool Copy projection");
        let result = results
            .iter()
            .find(|result| result.func_name.ends_with("unwrap_copy"))
            .expect("unwrap_copy verification result");
    ...
    #[test]
    fn verifier_proves_option_i64_unwrap_copy_projection() {
        let source = include_str!("../../tests/fixtures/mir_native_option_i64_unwrap.mimi");
        let tokens = Lexer::new(source).tokenize().expect("lex");
        let file = Parser::new(tokens).parse_file().expect("parse");
        let checked = crate::core::check_program(&file).expect("check");
        let program =
            MirProgram::from_checked_program(&checked).expect("canonical Option<i64>.unwrap MIR");
        let function = program
            .functions()
            .get(&crate::core::NodeId("function:unwrap_copy".into()))
            .expect("unwrap_copy MIR");
        ...
        let results = verify_program(&program, "option-i64-unwrap-source-hash".into())
            .expect("verifier should prove the i64 Copy projection");
        let result = results
            .iter()
            .find(|result| result.func_name.ends_with("unwrap_copy"))
            .expect("unwrap_copy verification result");
